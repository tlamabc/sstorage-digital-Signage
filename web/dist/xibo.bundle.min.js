...
const Common = __webpack_require__(/*! ../editor-core/common.js */ "./ui/src/editor-core/common.js");
const PlayerHelper = __webpack_require__(/*! ../helpers/player-helper.js */ "./ui/src/helpers/player-helper.js");

// LOCAL FUNCTIONS

// Check condition
function checkCondition(type, value, targetValue) {
  let isTopLevel = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : true;
  if (type === 'ne' && value === '') {
    return true;
    ...
    return true;
  } else if (type === 'gt' && !Number.isNaN(parseInt(targetValue)) && !Number.isNaN(parseInt(value)) && parseInt(targetValue) > parseInt(value)) {
    return true;
  } else if (type === 'lt' && !Number.isNaN(parseInt(targetValue)) && !Number.isNaN(parseInt(value)) && parseInt(targetValue) < parseInt(value)) {
    return true;
  } else if (type === 'egt' && !Number.isNaN(parseInt(targetValue)) && !Number.isNaN(parseInt(value)) && parseInt(targetValue) >= parseInt(value)) {
    return true;
  } else if (type === 'elt' && !Number.isNaN(parseInt(targetValue)) && !Number.isNaN(parseInt(value)) && parseInt(targetValue) <= parseInt(value)) {
    return true;
...
function createTableFromContext(dialog, context) {
  ...
  columns.push({
    ...
    data: function (data, type, row) {
      ...
      // Create checkbox
      return '<input type="checkbox" class="checkbox"' + ' data-member-id=' + data[context.id] + ' data-member-type="' + context.type + '" ' + checked + '>';
    }
  });
...
function assignLayoutToCampaign(url, layout, unassignLayout) {
  ...
  $.ajax({
    ...
    data: {
      layoutId: layout,
      unassignLayoutId: unassignLayout
    },
    success: XiboSubmitResponse
  });
}
...
function setFeatureGroupCheckboxState(triggerElement) {
  ...
  const countChecked = $featureGroup.find('input[name=\'features[]\']:checked').length;
  ...
  const countInheritChecked = $featureGroup.find('input.inherit-group:checked').length;
...
function setCheckboxState(count, countTotal, $selector, checkboxClass) {
  if (count <= 0) {
    $selector.find(checkboxClass).prop('checked', false).prop('indeterminate', false);
  } else if (count === countTotal) {
    $selector.find(checkboxClass).prop('checked', true).prop('indeterminate', false);
  } else {
    $selector.find(checkboxClass).prop('checked', false).prop('indeterminate', true);
  }
}
...
window.forms = {
  ...
  createFields: function (properties, targetContainer, targetId, playlistId) {
    let propertyGroups = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : [];
    let customClass = arguments.length > 5 ? arguments[5] : undefined;
    let prepend = arguments.length > 6 && arguments[6] !== undefined ? arguments[6] : false;
    for (const key in properties) {
      if (properties.hasOwnProperty(key)) {
        ...
        if (property.type === 'menuBoardCategorySelector') {
          property.menuBoardCategorySearchUrl = urlsForApi.menuBoard.categorySearch.url;
        ...
        // Colour format
        if (property.type === 'color' && property.format != '') {
          property.colorFormat = property.format;
        }

        // dashboards available services
        if (property.type === 'connectorProperties') {
          property.connectorPropertiesUrl = urlsForApi.connectorProperties.search.url.replace(':id', targetId) + '?propertyId=' + property.id;
        ...
        if (templates.forms.hasOwnProperty(property.type)) {
          // New field
          const $newField = $(templates.forms[property.type](Object.assign({}, property, {
            trans: typeof propertiesPanelTrans === 'undefined' ? {} : propertiesPanelTrans
          })));
          ...
          if (property.propertyGroupId) {
            ...
            const group = propertyGroups.find(group => group.id === property.propertyGroupId);

            // Only add to group if it exists
            if (group) {
              // Check if the group already exists in the DOM, if not create it
              if ($(targetContainer).find('#' + property.propertyGroupId).length) {
                // Set target container to be the group
                $targetContainer = $(targetContainer).find('#' + property.propertyGroupId + ' .field-container');
              } else {
                // Create the group and add it to the target container
                $targetContainer.append($(templates.forms.group({
                  id: group.id,
                  title: group.title,
                ...
                $targetContainer = $($(targetContainer).find('#' + property.propertyGroupId)).find('.field-container');
              }
            }
          }
          ...
          if (property.helpText) {
            ...
            if ($newField.find('.input-info-container .xibo-help-text').length === 0) {
              $newField.find('.input-info-container').append($(templates.forms.addOns.helpText({
          ...
          if (property.customPopOver) {
            $newField.find('.input-info-container').append($(templates.forms.addOns.customPopOver({
          ...
          if (property.playerCompatibility) {
            $newField.find('.input-info-container').append($(templates.forms.addOns.playerCompatibility(property.playerCompatibility)));
          }

          // Add date format helper popup when variant = dateFormat
          if (property.variant && property.variant === 'dateFormat') {
            $newField.find('label.control-label').append($(templates.forms.addOns.dateFormatHelperPopup({
  ...
  initFields: function (container, target, targetId) {
    ...
    // Dropdowns
    findElements('.dropdown-input-group', target).each(function (_k, el) {
      ...
      $dropdown.find('option[data-content]').each(function (_k, option) {
        ...
        $image.attr('src', assetDownloadUrl.replace(':assetId', $image.attr('src')));
    ...
    // Button group switch
    findElements('.button-switch-input-group', target).each(function (_k, el) {
      ...
      $buttonGroup.find('button').on('click', function (ev) {
        ...
        $buttonGroup.find('button').removeClass('selected');
    ...
    findElements('.dataset-order-clause', target).each(function (_k, el) {
      const $el = $(el);
      ...
      if (datasetId) {
        ...
        $.ajax({
          ...
          const $orderClauseHiddenInput = $el.find('#input_' + $el.data('order-id'));
          const orderClauseValues = $orderClauseHiddenInput.val() ? JSON.parse($orderClauseHiddenInput.val()) : [];

          // Update the hidden field with a JSON string
          // of the order clauses
          const updateHiddenField = function () {
            const orderClauses = [];
            $orderClauseFields.find('.order-clause-row').each(function (_index, el) {
              const $el = $(el);
              const orderClause = $el.find('.order-clause').val();
              const orderClauseDirection = $el.find('.order-clause-direction').val();
              if (orderClause) {
                orderClauses.push({
            ...
            // Update the hidden field with a JSON string
            $orderClauseHiddenInput.val(JSON.stringify(orderClauses)).trigger('change');
          };
          ...
          // Get template
          const orderClauseTemplate = formHelpers.getTemplate('dataSetOrderClauseTemplate');
    ...
    findElements('.dataset-column-selector', target).each(function (_k, el) {
      const $el = $(el);
      const datasetId = $el.data('depends-on-value');
      ...
      if (datasetId) {
        ...
        $.ajax({
          ...
          if ($colsOutContainer.length == 0 || $colsInContainer.length == 0) {
            return;
          }
          const $selectHiddenInput = $el.find('#input_' + $el.data('select-id'));
          const selectedValue = $selectHiddenInput.val() ? JSON.parse($selectHiddenInput.val()) : [];

          // Update the hidden field with a JSON string
          // of the order clauses
          const updateHiddenField = function () {
            let skipSave = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : false;
            const selectedCols = [];
            ...
            $.each(selectedCols, function (_index, col) {
              $el.append('<input type="hidden" class="temp" ' + 'name="dataSetColumnId[]" value="' + col + '" />');
            });

            // Update the hidden field with a JSON string
            $selectHiddenInput.val(JSON.stringify(selectedCols)).trigger('change', [{
              skipSave: skipSave
            }]);
          };
          ...
          const colAvailableTitle = datasetColumnSelectorTranslations.colAvailable;
          const colSelectedTitle = datasetColumnSelectorTranslations.colSelected;
          ...
          $.each(datasetColsOut, function (_index, col) {
            $columnsOut.append('<li class="li-sortable" id="' + col.dataSetColumnId + '">' + col.heading + '</li>');
          });

          // Populate the selected columns
          const $columnsIn = $el.find('#columnsIn');
          $.each(datasetColsIn, function (_index, col) {
            $columnsIn.append('<li class="li-sortable" id="' + col.dataSetColumnId + '">' + col.heading + '</li>');
          });
          if (!readOnlyMode) {
            ...
            $el.find('.li-sortable').on('dblclick', function (ev) {
              ...
              $this.appendTo($this.parent().is('#columnsIn') ? $columnsOut : $columnsIn);
              updateHiddenField();
            });
          }
    ...
    // Dataset field selector
    findElements('.dataset-field-selector', target).each(function (_k, el) {
      const $el = $(el);
      ...
      if (datasetId) {
        ...
        $.ajax({
          ...
          if ($dataTypeId.length > 0 && datasetCols.length > 0) {
            ...
            const fieldItems = getFieldItems(datasetCols, $dataTypeId.val()) || [];
            if (fieldItems.length > 0) {
              $select.find('option[value]').remove();
              $.each(fieldItems, function (_k, field) {
                $select.append($('<option value="' + field.heading + '">' + field.heading + '</option>'));
              });
    ...
    findElements('.dataset-filter-clause', target).each(function (_k, el) {
      const $el = $(el);
      const datasetId = $el.data('depends-on-value');
      ...
      if (datasetId) {
        ...
        $.ajax({
          ...
          const $filterClauseHiddenInput = $el.find('#input_' + $el.data('filter-id'));
          const filterClauseValues = $filterClauseHiddenInput.val() ? JSON.parse($filterClauseHiddenInput.val()) : [];

          // Update the hidden field with a JSON string
          // of the filter clauses
          const updateHiddenField = function () {
            const filterClauses = [];
            $filterClauseFields.find('.filter-clause-row').each(function (_index, el) {
              const $el = $(el);
              const filterClause = $el.find('.filter-clause').val();
              const filterClauseOperator = $el.find('.filter-clause-operator').val();
              const filterClauseCriteria = $el.find('.filter-clause-criteria').val();
              const filterClauseValue = $el.find('.filter-clause-value').val();
              if (filterClause) {
                filterClauses.push({
            ...
            // Update the hidden field with a JSON string
            $filterClauseHiddenInput.val(JSON.stringify(filterClauses)).trigger('change');
          };
          ...
          // Get template
          const filterClauseTemplate = formHelpers.getTemplate('dataSetFilterClauseTemplate');
          const filterOptions = datasetQueryBuilderTranslations.filterOptions;
          const filterOperatorOptions = datasetQueryBuilderTranslations.filterOperatorOptions;
          ...
          if (!readOnlyMode) {
            ...
            $el.on('change', 'select:not([type="hidden"]), input:not([type="hidden"])', function (ev) {
              updateHiddenField();
            });
          } else {
            forms.makeFormReadOnly($el);
    ...
    findElements('.ticker-tag-selector', target).each(function (_k, el) {
      const $el = $(el);
      ...
      if ($tagsOutContainer.length == 0 || $tagsInContainer.length == 0) {
        return;
      }
      const $selectHiddenInput = $el.find('#input_' + $el.data('select-id'));
      const tagsValue = $selectHiddenInput.val() ? JSON.parse($selectHiddenInput.val()) : [];
      const selectedTags = Object.keys(tagsValue);

      // Update the hidden field with a JSON string
      // of the tags
      const updateHiddenField = _.debounce(function () {
        let skipSave = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : false;
        const selectedTags = [];
        const tagsValue = $selectHiddenInput.val() ? JSON.parse($selectHiddenInput.val()) : [];
      ...
      const updateTagsStylesHiddenField = _.debounce(function () {
        ...
        $el.find('.ticker-tag-style').each(function (_index, tagContainer) {
          ...
          const $tagProperties = $(tagContainer).find('.ticker-tag-style-property');
          ...
          $tagProperties.find('select, input').each(function (_index, tagInput) {
            ...
            const value = $(tagInput).attr('type') === 'checkbox' ? $(tagInput).is(':checked') : $(tagInput).val();
            selectedTagStylesValue[tagId][inputType] = value;
          });
        });

        // Update the hidden field with a JSON string
        $selectHiddenInput.val(JSON.stringify(selectedTagStylesValue)).trigger('change');
      }, 100);

      // Clear existing fields
      $tagsOutContainer.empty();
      $tagsInContainer.empty();
      const tagAvailableTitle = tickerTagSelectorTranslations.tagAvailable;
      const tagSelectedTitle = tickerTagSelectorTranslations.tagSelected;
      ...
      $.each(tickerTagsOut, function (_index, tag) {
        const $tag = $('<li class="li-sortable" id="' + tag + '" data-tag-type="' + tickerTagsMap[tag] + '">' + tickerTagSelectorTranslations[tag] + '</li>').data('tag-type', tickerTagsMap[tag]);
        $tagsOut.append($tag);
      });
      ...
      $.each(tickerTagsIn, function (_index, tag) {
        const $tag = $('<li class="li-sortable" id="' + tag + '" data-tag-type="' + tickerTagsMap[tag] + '">' + tickerTagSelectorTranslations[tag] + '</li>');
        $tagsIn.append($tag);
      });
      if (!readOnlyMode) {
        // Setup lists drag and sort ( with double click )
        ...
        $el.find('.li-sortable').on('dblclick', function (ev) {
          const $this = $(ev.currentTarget);
          $this.appendTo($this.parent().is('#tagsIn') ? $tagsOut : $tagsIn);
          updateHiddenField();
        });
      }
    ...
    findElements('.dataset-column-style-selector', target).each(function (_k, el) {
      const $el = $(el);
      const datasetId = $el.data('depends-on-value');
      ...
      if (datasetId) {
        ...
        $.ajax({
          ...
          const $selectHiddenInput = $el.find('#input_' + $el.data('select-id'));
          const colsValue = $selectHiddenInput.val() ? JSON.parse($selectHiddenInput.val()) : [];
          ...
          const updateHiddenField = _.debounce(function () {
            ...
            selectedCols.forEach(col => {
              ...
              // Update hidden field on input change
              $newStyle.off().on('change inputChange', function () {
          ...
          const updateColStylesHiddenField = _.debounce(function () {
            ...
            $el.find('.dataset-col-style').each(function (_index, colContainer) {
              ...
              const $colProperties = $(colContainer).find('.dataset-col-style-property');
              ...
              $colProperties.find('select, input').each(function (_index, colsInput) {
                const inputType = $(colsInput).data('dataset-col-style-input');
                const value = $(colsInput).attr('type') === 'checkbox' ? $(colsInput).is(':checked') : $(colsInput).val();
                selectedColStylesValue[colId][inputType] = value;
              });
            });

            // Update the hidden field with a JSON string
            $selectHiddenInput.val(JSON.stringify(selectedColStylesValue)).trigger('change');
          }, 100);

          // Clear existing fields
          $colsOutContainer.empty();
          $colsInContainer.empty();
          const colsAvailableTitle = datasetColStyleSelectorTranslations.colAvailable;
          const colsSelectedTitle = datasetColStyleSelectorTranslations.colSelected;
          ...
          $.each(colsOut, function (_index, col) {
            ...
            const $col = $('<li class="li-sortable" id="' + columnObj.dataSetColumnId + '" data-col-type="' + columnObj.dataTypeId + '">' + columnObj.heading + '</li>').data('col-type', columnObj.dataTypeId);
            $colsOut.append($col);
          });
          ...
          $.each(colsIn, function (_index, col) {
            const columnObj = getColById(col);
            const $col = $('<li class="li-sortable" id="' + columnObj.dataSetColumnId + '" data-col-type="' + columnObj.dataTypeId + '">' + columnObj.heading + '</li>');
            $colsIn.append($col);
          });
          if (!readOnlyMode) {
            // Setup lists drag and sort ( with double click )
            ...
            $el.find('.li-sortable').on('dblclick', function (ev) {
              const $this = $(ev.currentTarget);
              $this.appendTo($this.parent().is('#colsIn') ? $colsOut : $colsIn);
              updateHiddenField();
            });
          }
    ...
    findElements('.languageSelect', target).each(function (_k, el) {
      ...
      if (isMomentLocaleSelect) {
        ...
        for (let index = 0; index < momentLocales.length; index++) {
          ...
          // Create new option
          const $newOption = $(`<option value="${locale}">${locale} - ${name}</option>`);
    ...
    // Playlist mixer
    findElements('.playlist-mixer', target).each(function (_k, el) {
      const $el = $(el);
      ...
      function subplaylistInitRow($form, $row) {
        ...
        if ($select.data('fieldId')) {
          $.ajax({
            method: 'GET',
            url: urlsForApi.playlist.get.url + '?playlistId=' + $select.data('fieldId'),
            success: function (response) {
              if (response.data && response.data.length > 0) {
                // Append our initial option
                $select.append('<option value="' + response.data[0].playlistId + '" data-tags="' + response.data[0].tags + '" selected>' + response.data[0].name + '</option>');
                subplaylistInitSelect2($form, $select);
              } else {
                // No permissions.
                $select.parent().append('<input type="hidden" value="' + $select.data('fieldId') + '" name="subPlaylistId[]">' + '<span title="' + playlistMixerTranslations.noPermission + '">' + '<i class="fa fa-lock"></i>&nbsp;' + playlistMixerTranslations.playlistId + ' ' + $select.data('fieldId') + '</span>');
                $select.remove();
              }
      ...
      function subplaylistInitSelect2($form, $el) {
        $el.select2({
          dropdownAutoWidth: true,
          ajax: {
            url: urlsForApi.playlist.get.url + '?notPlaylistId=' + $form.data('playlistId'),
            dataType: 'json',
            delay: 250,
      ...
      const $mixerHiddenInput = $el.find('#input_' + $el.data('mixer-id'));
      const mixerValues = $mixerHiddenInput.val() ? JSON.parse($mixerHiddenInput.val()) : [];
      ...
      const updateHiddenField = function () {
        const mixerItems = [];
        $playlistItemsContainer.find('.subplaylist-item-row').each(function (_index, el) {
          const $el = $(el);
          const playlistId = $el.find('.subplaylist-id').val();
          const spots = $el.find('.subplaylist-spots').val();
          const spotLength = $el.find('.subplaylist-spots-length').val();
          const spotFill = $el.find('.subplaylist-spots-fill').val();
          if (playlistId) {
            mixerItems.push({
              rowNo: _index + 1,
        ...
        // Update the hidden field with a JSON string
        $mixerHiddenInput.val(JSON.stringify(mixerItems)).trigger('xiboInputChange');
      };
      ...
      // Add header template
      const containerTemplate = formHelpers.getTemplate('subPlaylistContainerTemplate');
      $playlistItemsContainer.append(containerTemplate({
        trans: playlistMixerTranslations
      }));

      // Get template
      const subPlaylistFormTemplate = formHelpers.getTemplate('subPlaylistFormTemplate');
      if (mixerValues.length == 0) {
        ...
        $playlistItemsContainer.find('.subplaylist-items-content').append(subPlaylistFormTemplate(context));
      } else {
        // For each of the existing codes, create form components
      ...
      $playlistItemsContainer.on('click', '.subplaylist-item-btn', function (e) {
        ...
        if ($(e.currentTarget).find('i').hasClass('fa-plus')) {
          ...
          const $newRow = $(subPlaylistFormTemplate(context)).appendTo($playlistItemsContainer.find('.subplaylist-items-content'));
      ...
      // Initialise all rows
      $playlistItemsContainer.find('.subplaylist-item-row').each(function (_index, element) {
    ...
    // Code editor
    findElements('.xibo-code-input', target).each(function (_k, el) {
      const $container = $(el);
      ...
      const codeTypeFunc = CodeMirror[codeType] ? CodeMirror[codeType] : CodeMirror.html;
      new CodeMirror.EditorView({
        doc: inputValue,
        extensions: [CodeMirror.basicSetup, CodeMirror.keymap.of([CodeMirror.indentWithTab]), codeTypeFunc(), CodeMirror.EditorView.updateListener.of(update => {
          if (update.docChanged && update.state.doc.toString() != $textArea.val()) {
      ...
      const toggleEditor = function () {
        ...
        if (isOpened) {
          ...
          $codeFSOverlay.removeClass('custom-overlay').addClass('code-fs-overlay custom-overlay-clone').appendTo($('body')).on('click', toggleEditor);
          $codeFSOverlay.show();
          ...
          // Add placeholder
          const $codeFSPlaceholder = $('<div>').addClass('code-fs-placeholder').css('height', codeEditorHeight);
    ...
    // Colour picker
    findElements('.colorpicker-input.colorpicker-element', target).each(function (_k, el) {
      // Init the colour picker
      $(el).colorpicker({
        container: $(el).find('.picker-container'),
        align: 'left',
        format: $(el).data('colorFormat') !== undefined ? $(el).data('colorFormat') : false
      });
    ...
    // Colour gradient
    findElements('.color-gradient', target).each(function (_k, el) {
      ...
      const $inputElements = $(el).find('[name]:not(.color-gradient-hidden).element-property');
      const $hiddenInput = $(el).find('.color-gradient-hidden.element-property');
      const $gradientType = $inputElements.filter('[name="gradientType"]');
      const $gradientAngle = $inputElements.filter('[name="gradientAngle"]');
      const $gradientColor1 = $inputElements.filter('[name="gradientColor1"]');
      const $gradientColor2 = $inputElements.filter('[name="gradientColor2"]');

      // Load values into inputs
      if ($hiddenInput.val() != '') {
        const initialValue = JSON.parse($hiddenInput.val());
        $gradientType.val(initialValue.type).trigger('change');
        $gradientColor1.parents('.colorpicker-input').colorpicker('setValue', initialValue.color1);
        $gradientColor2.parents('.colorpicker-input').colorpicker('setValue', initialValue.color2);
      ...
      const updateFields = function () {
        if ($gradientColor1.val() == '') {
          $gradientColor1.parents('.colorpicker-input').colorpicker('setValue', defaults.color1);
        }
        if ($gradientColor2.val() == '') {
          $gradientColor2.parents('.colorpicker-input').colorpicker('setValue', defaults.color2);
        }
    ...
    findElements('.dateControl.date:not(.datePickerHelper)', target).each(function (_k, el) {
      if (calendarType == 'Jalali') {
        initDatePicker($(el), systemDateFormat, jsDateOnlyFormat, {
    ...
    findElements('.dateControl.dateTime:not(.datePickerHelper)', target).each(function (_k, el) {
      const enableSeconds = dateFormat.includes('s');
      const enable24 = !dateFormat.includes('A');
      if (calendarType == 'Jalali') {
        initDatePicker($(el), systemDateFormat, jsDateFormat, {
          timePicker: {
            enabled: true,
            second: {
        ...
        initDatePicker($(el), systemDateFormat, jsDateFormat, {
          ...
          altFormat: $(el).data('customFormat') ? $(el).data('customFormat') : jsDateFormat
        });
      }
    });

    // Date picker - month only
    findElements('.dateControl.month:not(.datePickerHelper)', target).each(function (_k, el) {
      if (calendarType == 'Jalali') {
        initDatePicker($(el), systemDateFormat, jsDateFormat, {
          format: $(el).data('customFormat') ? $(el).data('customFormat') : 'MMMM YYYY',
          viewMode: 'month',
          dayPicker: {
            enabled: false
          },
        ...
        initDatePicker($(el), systemDateFormat, jsDateFormat, {
          plugins: [new flatpickrMonthSelectPlugin({
            ...
            altFormat: $(el).data('customFormat') ? $(el).data('customFormat') : 'MMMM Y',
    ...
    findElements('.dateControl.time:not(.datePickerHelper)', target).each(function (_k, el) {
      const enableSeconds = dateFormat.includes('s');
      if (calendarType == 'Jalali') {
        initDatePicker($(el), systemTimeFormat, jsTimeFormat, {
          onlyTimePicker: true,
          format: jsTimeFormat,
          timePicker: {
            second: {
        ...
        initDatePicker($(el), systemTimeFormat, jsTimeFormat, {
          ...
          enableSeconds: enableSeconds,
          time_24hr: true,
          altFormat: $(el).data('customFormat') ? $(el).data('customFormat') : jsTimeFormat
        });
      }
    });

    // Rich text input
    findElements('.rich-text', target).each(function (_k, el) {
      formHelpers.setupCKEditor(container, $(el).attr('id'), true, null, false, true);
    });

    // World clock control
    findElements('.world-clock-control', target).each(function (_k, el) {
      ...
      function configureMultipleWorldClocks(container) {
        ...
        const worldClocks = $hiddenInput.attr('value') ? JSON.parse($hiddenInput.attr('value')) : [];
      ...
      function initClockRows(container) {
        ...
        $(container).find('.localSelect select.form-control').each(function (_k, el2) {
          makeLocalSelect($(el2), $(container).hasClass('modal') ? $(container) : $('body'));
        });
        ...
        $(container).find('input[type="text"], select').on('change', function () {
          updateClocksHiddenInput(container);
        });
      }
    ...
    // Font selector
    findElements('.font-selector', target).each(function (_k, el) {
          ...
          if (res.data !== undefined && res.data.length > 0) {
            $.each(res.data, function (_index, element) {
              if ($el.data('value') === element.familyName) {
                $el.append($('<option value="' + element.familyName + '" selected>' + element.name + '</option>'));
                ...
                $el.append($('<option value="' + element.familyName + '">' + element.name + '</option>'));
              }
            });
          }
    ...
    // Snippet selector
    findElements('.snippet-selector', target).each(function (_k, el) {
      // Get select element
      const $select = $(el).find('select');
      ...
      const setupSnippets = function ($select) {
        formHelpers.setupSnippetsSelector($select, function (e) {
          ...
          const ckeditorInstance = formHelpers.getCKEditorInstance('input_' + targetId + '_' + targetFieldId);
          if (ckeditorInstance) {
            // CKEditor
            formHelpers.insertToCKEditor('input_' + targetId + '_' + targetFieldId, text);
          } else if ($targetField.hasClass('code-input')) {
            // Code editor
            const editor = CodeMirror.EditorView.findFromDOM($targetField.next('.code-input-editor-container')[0]);
            ...
            $targetField.val(previousText.substring(0, cursorPosition) + text + previousText.substring(cursorPosition));
      ...
      const setupMediaSnippets = function ($select) {
        ...
        formHelpers.setupMediaSelector($select, function (e) {
          const value = $(e.currentTarget).val();
          ...
          // Text to be inserted
          const textURL = urlsForApi.library.download.url.replace(':id', value);
      ...
      if (snippetMode == 'dataSet') {
            ...
            success: function (response) {
              ...
              if (data && data.columns && data.columns.length > 0) {
                ...
                $.each(data.columns, function (_index, col) {
                  $select.append($('<option data-snippet-type="' + col.dataType + '" value="' + col.heading + '|' + col.dataSetColumnId + '">' + col.heading + '</option>'));
                });
        ...
        // Get request path
        const requestPath = urlsForApi.widget.getDataType.url.replace(':id', targetId);
          ...
          success: function (response) {
            if (response && response.fields && response.fields.length > 0) {
              ...
              $.each(response.fields, function (_index, element) {
                $select.append($('<option data-snippet-type="' + element.type + '" value="' + element.id + '">' + element.title + '</option>'));
              });
    ...
    // Effect selector
    findElements('.effect-selector', target).each(function (_k, el) {
      ...
      $.each(effects, function (_index, element) {
        ...
        $appendTo.append($('<option value="' + element.effect + '" data-optgroup="' + element.group + '">' + effectsTranslations[element.effect] + '</option>'));
      });
      ...
      if ($(el).next().find('input[name="speed"]').length === 1) {
        ...
        speedInput.val(setEffectSpeed(currentSelected, currentSelected, speedInput.val()));
        $el.on('select2:select', function (e) {
          const data = e.params.data;
          const effect = data.id;
          speedInput.val(setEffectSpeed(currentSelected, effect, speedInput.val()));
        });
      }
    });

    // Font selector
    findElements('.menu-board-category-selector', target).each(function (_k, el) {
      // Replace search url with the menuId
      const $el = $(el);
      const $elSelect = $(el).find('select');
      const dependsOnName = $el.data('depends-on');
      const $menu = $(container).find('[name=' + dependsOnName + ']');

      // Get menu value
      const menuId = $menu.val();
      if (menuId != '') {
        // Replace URL
        $elSelect.data('search-url', $elSelect.data('search-url-base').replace(':id', menuId));
    ...
    // Select2 dropdown
    findElements('.select2-hidden-accessible', target).each(function (_k, el) {
      const $el = $(el);
    ...
    findElements('input[data-role=panelTagsInput]', target).each(function (_k, el) {
      const self = $(el);
    ...
    if (!target) {
      $(container).find('.xibo-form-input[data-depends-on]').each(function (_k, el) {
        const $target = $(el);
        ...
        $(container).find(base).on('change', function (ev) {
          ...
          if ($base.is('select')) {
            // Get selected option
            const $selectedOption = $base.find('option:selected');
          ...
          if ($target.children('input[type="checkbox"]').length > 0) {
            ...
            $target.children('input[type="checkbox"]').prop('checked', valueToSet);
            ...
            } else if (valueToSet !== null && valueToSet !== undefined && Color(valueToSet).valid) {
              // Add the color to the color picker
              $target.find('input').colorpicker('setValue', valueToSet);

              // Also update the background color of the input group
              $target.find('.input-group-addon').css('background-color', valueToSet);
            }
  ...
  handleFormReplacements: function (container, baseObject) {
    ...
    $(container).find('.xibo-form-input > [title], .xibo-form-btn[title]').each(function (_idx, el) {
      const $element = $(el);
    ...
    $(container).find('.xibo-form-input > *, .xibo-form-btn').each(function (_idx, el) {
      const $element = $(el);
  ...
  setConditions: function (container, baseObject, targetId) {
    let isTopLevel = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : true;
    $(container).find('.xibo-form-input[data-visibility]').each(function (_idx, el) {
      let visibility = $(el).data('visibility');

      // Handle replacements for visibilty rules
      visibility = JSON.parse(JSON.stringify(visibility).replace(/\$(.*?)\$/g, function (_m, group) {
      ...
      const isConditionTargetExists = function (test) {
        if (test?.field && targetId) {
          const $conditionTargetElem = $(container).find(`[name="${test.field}"]`);
        ...
        if (test.conditions.length > 0 && targetId) {
          for (let i = 0; i < test.conditions.length; i++) {
            ...
            const $conditionTargetElem = $(container).find(`[name="${conditionField}"]`);
      ...
      const buildTest = function (test, $testContainer) {
        ...
        const checkTest = function () {
          ...
          for (let i = 0; i < testConditions.length; i++) {
            ...
            const $conditionTarget = $(container).find(fieldId);

            // Get condition target value based on type
            const conditionTargetValue = $conditionTarget.length !== 0 && $conditionTarget.attr('type') === 'checkbox' ? $conditionTarget.is(':checked') : $conditionTarget.val();
            const newTestResult = checkCondition(condition.type, condition.value, conditionTargetValue, isTopLevel);
  ...
  makeFormReadOnly: function ($formContainer) {
    ...
    $formContainer.find('input, select, ' + 'textarea, button:not(.copyTextAreaButton)').attr('disabled', 'disabled');

    // Disable color picker plugin
    $formContainer.find('.colorpicker-input i.input-group-addon').off('click');

    // Hide buttons
    $formContainer.find('button:not(.copyTextAreaButton), ' + '.date-clear-button').hide();
  ...
  reloadRichTextFields: function ($formContainer) {
    $formContainer.find('.rich-text').each(function (_k, el) {
      ...
      // Reload
      formHelpers.setupCKEditor($formContainer, elId, true, null, false, true);
  ...
  * @param {callback} reloadWidgetCallback
   */
  createFallbackDataForm: function (dataType, container, fallbackData, widget, reloadWidgetCallback) {
    ...
    const createField = function (field, data) {
      ...
      // Set date variant
      if (['datetime', 'date', 'time', 'month'].indexOf(fieldClone.type) != -1) {
        fieldClone.variant = fieldClone.type === 'datetime' ? 'dateTime' : fieldClone.type;
      ...
      if ($newField && $newField.is('[data-is-required="true"]')) {
        $newField.find('label').append(`<span class="ml-1"
            title=${fallbackDataTrans.requiredField}>*</span>`);
      }
      return $newField;
    };
    const updateRecordPreview = function ($record, recordData) {
      const $previewsContainer = $record.find('.fallback-data-record-previews');
      ...
      dataFields.forEach(field => {
        if (recordData && recordData[field.id]) {
          ...
          $record.find('.fallback-data-record-previews').append($recordPreview);
        }
      });
    };
    const saveRecord = function ($record) {
      ...
      dataFields.forEach(field => {
        ...
        const required = $field.parents('.fallback-property').is('[data-is-required="true"]');
        if (value == '' && required) {
      ...
      if (recordId) {
        widget.editFallbackDataRecord(recordId, recordData, displayOrder).then(_res => {
          if (_res.success) {
            updateRecord();
          }
        });
      } else {
        widget.addFallbackData(recordData, displayOrder).then(_res => {
          if (_res.success) {
    ...
    const deleteRecord = function ($record) {
      const recordId = $record.data('recordId');
      if (recordId) {
        widget.deleteFallbackDataRecord(recordId).then(_res => {
          if (_res.success) {
            // Remove object
            $record.remove();
            reloadWidgetCallback();
    ...
    const createRecord = function () {
      ...
      dataFields.forEach(field => {
        ...
        // New field input
        $recordContainer.find('.fallback-data-record-fields').append($newField);
      });
      ...
      $recordContainer.find('button').on('click', function (ev) {
        const actionType = $(ev.currentTarget).data('action');
    ...
    const $recordsContainer = $(container).find('.fallback-data-records');
    ...
    if (fallbackData) {
      ...
      fallbackData.forEach(data => {
        $recordsContainer.append(createRecord(data));
      });
    ...
    $(container).find('[data-action="add-new-record"]').on('click', function () {
      $recordsContainer.append(createRecord());
    ...
    $recordsContainer.sortable({
      ...
      update: function () {
        ...
        widget.saveFallbackDataOrder(records).then(_res => {
          if (_res.success) {
            reloadWidgetCallback();
          }
        });
  ...
  */
  ...
  permissionsFormOpen: function () {
    ...
    // Bind our filter
    grid.find('.permissionsTableFilter form input, .permissionsTableFilter form select').on('change', function () {
      table.ajax.reload();
    });
  },
  ...
  permissionsMultiFormOpen: function (dialog) {
    ...
    // Bind our filter
    $grid.find('.permissionsMultiTableFilter form input, ' + '.permissionsMultiTableFilter form select').on('change', function () {
      table.ajax.reload();
    });
  },
  ...
  */
  ...
  mediaDisplayGroupFormCallBack: function () {
    ...
    mediaTable.on('draw', function (e, settings) {
      ...
      // Clicky on the +spans
      $('.assignItem:not(.disabled)', '#mediaAssignments').on('click', function (ev) {
    ...
    // Bind to the filter
    $('#mediaAssignments').closest('.XiboGrid').find('.FilterDiv input, .FilterDiv select').on('change', function () {
      mediaTable.ajax.reload();
    });
  },
  ...
  layoutFormCallBack: function () {
    ...
    // Bind to the filter
    $('#layoutAssignments').closest('.XiboGrid').find('.FilterDiv input, .FilterDiv select').on('change', function () {
  ...
  tagsWithValues: function (formId) {
    ...
    let tagOptions = [];
    let tagIsRequired = 0;
    const formSelector = '#' + formId + ' input#tags' + ', #' + formId + ' input#tagsToAdd';
              ...
              if (response.data.tag != null) {
                ...
                if (tagOptions != null && tagOptions != []) {
                  ...
                  if ($('#tagValue option[value=""]').length <= 0) {
                    $('#tagValue').append($('<option></option>').attr('value', '').text(''));
                  }
                  $.each(tagOptions, function (key, value) {
                    if ($('#tagValue option[value=' + value + ']').length <= 0) {
                      $('#tagValue').append($('<option></option>').attr('value', value).text(value));
                    }
                  });
    ...
    $('#tagValue').on('change', function (e) {
      ...
      if (tagN != '') {
        if (tagIsRequired === 0 || tagIsRequired === 1 && $(e.currentTarget).val() !== '') {
  ...
  */
  featureAclFormOpen: function (dialog) {
    ...
    $(dialog).find('input.feature-select-all').on('click', function (ev) {
      ...
      $(ev.currentTarget).closest('tbody.feature-group').find('input[name=\'features[]\']').prop('checked', $(ev.currentTarget).is(':checked'));
    });
  },
  ...
  validateForm: function (form, container, options) {
      ...
      invalidHandler: function (event, validator) {
        ...
        validator.errorList.forEach(error => {
          ...
          $(element).parents('.tab-pane').each((_id, tab) => {
            $('.nav-item a.nav-link[href="#' + $(tab).attr('id') + '"]:not(.active)').addClass('has-error');
          });
        });
...
$(function () {
    ...
    if (scheduleCriteria.types) {
      ...
      if (typeData) {
        ...
        if (metrics.length === 1) {
          ...
          $metricField = createReadonlyAndHiddenFields(metric.name, metric.id, 'criteria_metric[]');
        } else {
  ...
  function updateMetricsFieldAsText($row) {
    const $metricLabel = $row.find('label[for="criteria_metric[]"]');
    const $metricInput = $('<input class="form-control" name="criteria_metric[]"' + ' type="text" value="" />');
  ...
  $('body').on('change', '#scheduleCriteriaFields select[name="criteria_metric[]"]', function (e) {
    ...
    if (scheduleCriteria && selectedType) {
      const typeData = scheduleCriteria.types.find(t => t.id === selectedType);
      if (typeData) {
        const metrics = typeData.metrics;
        const metricData = metrics.find(m => m.id === selectedMetric);
  ...
  function updateValueField($row, values) {
    ...
    if (values.inputType === 'dropdown') {
      ...
      if (values.values.length === 1) {
        ...
        $valueLabel.append(createReadonlyAndHiddenFields(value.title, value.id, 'criteria_value[]'));
      } else {
        // change to dropdown and populate
        const $valueSelect = $('<select class="form-control" name="criteria_value[]"></select>');
  ...
  function updateValueFieldAsText($row) {
    const $valueLabel = $row.find('label[for="criteria_value[]"]');
    const $valueInput = $('<input class="form-control" name="criteria_value[]"' + ' type="text" value="" />');
  ...
  if ($('#Calendar').length > 0) {
    ...
    const navigateToCalendarDate = function () {
      if (calendar != undefined) {
        const selectedDate = moment(moment($('#fromDt').val()).format(systemDateFormat));
      ...
      if (calendar != undefined) {
        ...
        if (range === 'custom') {
          $('#fromDt, #toDt').on('change', function () {
            ...
            const from = moment(moment($('#fromDt').val()).startOf('day').format(systemDateFormat));
            const to = moment(moment($('#toDt').val()).startOf('day').format(systemDateFormat));
            const diff = to.diff(from, 'days');
            if (diff < 1) {
              calendar.options.view === 'agenda' ? calendar.view('agenda') : calendar.view('day');
    ...
    initDatePicker($('#dateInput'), systemDateFormat, jsDateOnlyFormat, pickerOptions, navigateToCalendarDatePicker, false // clear button
    );
    ...
    const options = {
      ...
      onBeforeEventsLoad: function (done) {
        ...
        if (this.options.view !== 'agenda') {
          ...
          // Serialise
          const displayGroups = $('select[name="displayGroupIds[]"').serialize();
          const displaySpecificGroups = $('select[name="displaySpecificGroupIds[]"').serialize();
          ...
          const nameRegEx = 'useRegexForName=' + $('#useRegexForName').is('checked');
          const nameLogicalOperator = $('#logicalOperatorName').serialize();
          !displayGroups && !displayLayouts && !displaySpecificGroups ? $calendarErrorMessage.show() : $calendarErrorMessage.hide();
          let url = calendarOptions.eventSource;

          // Append the selected filters
          url += '?' + displayLayouts + '&' + eventTypes + '&' + geoAware + '&' + recurring + '&' + name + '&' + nameRegEx + '&' + nameLogicalOperator;
          ...
          getJsonRequestControl = $.getJSON(url, params).done(function (data) {
            events = data.result;
            if (done != undefined) {
              done();
            }
          ...
          $('.time-picker-step-btn').off().on('click', function (ev) {
            $timePicker.slider('setValue', $timePicker.slider('getValue') + $(ev.currentTarget).data('step'));
            calendar.view();
          });
          ...
          if (!isShowAll) {
            ...
            $('select[name="displayGroupIds[]"] option, ' + 'select[name="displaySpecificGroupIds[]"] option').each(function (_idx, el) {
              const $self = $(el);
          ...
          const url = calendarOptions.agendaLink.replace(':id', selectedDisplayGroup);
          const dateMoment = moment(this.options.position.start.getTime() / 1000, 'X');
          const timeFromSlider = $('#timePickerSlider').length ? $('#timePicker').slider('getValue') : 0;
          const timeMoment = moment(timeFromSlider * 60, 'X');
          ...
          if (isShowAll) {
            ...
            getJsonRequestControl = $.getJSON(url, params).done(function (data) {
              ...
              if (!jQuery.isEmptyObject(data.data) && data.data.events != undefined && data.data.events.length > 0) {
                ...
                noEvents = false;
                if ($('#geoLatitude').val() != undefined && $('#geoLatitude').val() != '' && $('#geoLongitude').val() != undefined && $('#geoLongitude').val() != '') {
                  // eslint-disable-next-line max-len
                  ...
                  events['results'][String(selectedDisplayGroup)]['events'] = filterEventsByLocation(
                  // eslint-disable-next-line max-len
...
function createReadonlyAndHiddenFields(readonlyValue, hiddenValue, hiddenName) {
...
function updateConditionField($row, conditions, selectedCondition) {
  ...
  if (conditions.length === 1) {
    ...
    $conditionLabel.append(createReadonlyAndHiddenFields(condition.name, condition.id, 'criteria_condition[]'));
  } else {
...
window.setupScheduleForm = function (dialog) {
  ...
  // Events on change
  $('#recurrenceType, ' + '#eventTypeId, ' + '#dayPartId, ' + '#campaignId, ' + '#actionType, ' + '#fullScreenCampaignId, ' + '#relativeTime, ' + '#syncTimezone', dialog).on('change', function (ev) {
  ...
  $('a[data-toggle="tab"]', dialog).on('shown.bs.tab', function (e) {
    ...
    const fromDt = $fromDt.val() === null || $fromDt.val() === '' ? moment() : moment($fromDt.val());
    const $recurrenceMonthlyRepeatsOn = $(dialog).find('select[name=recurrenceMonthlyRepeatsOn]');
    const $dayOption = $('<option value="0">' + $recurrenceMonthlyRepeatsOn.data('transDay').replace('[DAY]', fromDt.format('Do')) + '</option>');
    const $weekdayOption = $('<option value="1">' + $recurrenceMonthlyRepeatsOn.data('transWeekday').replace('[POSITION]', nth(Math.ceil(fromDt.date() / 7))).replace('[WEEKDAY]', fromDt.format('dddd')) + '</option>');
  ...
  // eslint-disable-next-line max-len
  $('#scheduleAddForm, #scheduleEditForm, #scheduleDeleteForm, #scheduleRecurrenceDeleteForm').on('submit', function (e) {
    e.preventDefault();
  ...
  const $scheduleEditForm = $(dialog).find('#scheduleEditForm, #scheduleEditSyncForm');
  if ($scheduleEditForm.length > 0) {
    // Add a button for duplicating this event
    let $button = $('<button>').addClass('btn btn-info').attr('id', 'scheduleDuplateButton').html(translations.duplicate).on('click', function () {
...
window.beforeSubmitScheduleForm = function (form) {
  ...
  checkboxes.each(function (index, el) {
    $(el).parent().find('[type="hidden"]').val($(el).is(':checked') ? '1' : '0');
  });
  $('[data-toggle="popover"]').popover();
  form.submit();
};
 ...
 */
window.fullscreenBeforeSubmit = function (form, callBack) {
  let populateHiddenFields = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : true;
  const eventTypeId = form.find('[name="eventTypeId"]').val();
  const data = {
    id: eventTypeId == 7 ? form.find('[name="mediaId"]').val() : form.find('[name="playlistId"]').val(),
    type: eventTypeId == 7 ? 'media' : 'playlist',
    layoutDuration: eventTypeId == 7 ? form.find('[name="layoutDuration"]').val() : null,
    resolutionId: form.find('[name="resolutionId"]').select2('data').length > 0 ? form.find('[name="resolutionId"]').select2('data')[0].id : null,
  ...
  $.ajax({
    ...
    if (!response.success) {
      SystemMessageInline(response.message === '' ? translations.failure : response.message, form.closest('.modal'));
    }
...
const processScheduleFormElements = function (el, dialog) {
  ...
  switch (el.attr('id')) {
    case 'recurrenceType':
      ...
      const repeatControlGroupMonthDisplay = fieldVal !== 'Month' ? 'none' : '';
    ...
    case 'eventTypeId':
      ...
      const layoutControlDisplay = fieldVal == 2 || fieldVal == 6 || fieldVal == 7 || fieldVal == 8 || fieldVal == 10 ? 'none' : '';
      ...
      const maxPlaysControlDisplay = fieldVal == 2 || fieldVal == 6 || fieldVal == 10 ? 'none' : '';
      const mediaScheduleControlDisplay = fieldVal == 7 ? '' : 'none';
      const playlistScheduleControlDisplay = fieldVal == 8 ? '' : 'none';
      const playlistMediaScheduleControlDisplay = fieldVal == 7 || fieldVal == 8 ? '' : 'none';
      relativeTimeControlDisplay = fieldVal == 2 || !relativeTime ? 'none' : '';
    ...
    case 'dayPartId':
      ...
      endTimeControlDisplay = meta.isCustom === 0 || relativeTime ? 'none' : '';
      startTimeControlDisplay = meta.isAlways === 1 || relativeTime ? 'none' : '';
      const repeatsControlDisplay = meta.isAlways === 1 ? 'none' : '';
      const reminderControlDisplay = meta.isAlways === 1 ? 'none' : '';
      relativeTimeControlDisplay = meta.isCustom === 0 || !relativeTime ? 'none' : '';
    ...
    case 'actionType':
      ...
      const layoutCodeControl = fieldVal == 'navLayout' && el.is(':visible') ? '' : 'none';
    ...
    case 'relativeTime':
      ...
      const datePickerEndControlDisplay = $(el).is(':checked') || $('#eventTypeId', dialog).val() == 2 ? 'none' : '';
...
const setupSelectForSchedule = function (dialog) {
  ...
  if ([undefined, ''].indexOf($displaySelect.data('initialKey')) == -1 && $(dialog).find('form').data('setDisplaysFromGridFilters')) {
    ...
    $.ajax({
      ...
      data.data.forEach(object => {
        const option = new Option(object[$displaySelect.data('textProperty')], object[$displaySelect.data('idProperty')], true, true);
        $displaySelect.append(option);
      });

      // Trigger change but skip auto save
      $displaySelect.trigger('change', [{
        skipSave: true
      }]);
  ...
  $('#syncGroupId', dialog).on('select2:select', function (event) {
    ...
    const eventId = dialog.find('form').data().eventSyncGroupId == $target.select2('data')[0].id ? dialog.find('form').data().eventId : null;
    $.ajax({
      type: 'GET',
      url: dialog.find('form').data().fetchSyncDisplays.replace(':id', $target.select2('data')[0].id),
      cache: false,
      dataType: 'json',
      data: {
        eventId: eventId
      ...
      dialog.find('#contentSelectorTable tbody').html('').append(templates.calendar.syncEventContentSelector({
        ...response.data,
        ...{
      ...
      dialog.find('.pagedSelect select.form-control.syncContentSelect').each(function (_idx, el) {
        makePagedSelect($(el), '#' + formId);
      });
      dialog.find('.pagedSelect select.form-control.syncContentSelect').on('select2:select', function (ev) {
        if ($(ev.currentTarget).data().displayId === $(ev.currentTarget).data().leadDisplayId) {
      ...
      dialog.find('#setMirrorContent').on('click', function (ev) {
        ...
        const leadLayoutId = $('#layoutId_' + leadDisplayId).select2('data')[0].id;
        dialog.find('.pagedSelect select.form-control.syncContentSelect').not('#layoutId_' + leadDisplayId).each(function (_idx, el) {
...
const configureCriteriaFields = function (dialog) {
  ...
  const updateMetricsField = function ($row, typeId, selectedMetric, elementValue, selectedCondition) {
    const $metricLabel = $row.find('label[for="criteria_metric[]"]');
    let $metricField;
    if (typeId === 'custom') {
  ...
  const updateValueField = function ($row, metric, elementValue) {
    ...
    if (metric.values && metric.values.inputType === 'dropdown') {
      ...
      if (metric.values.values.length === 1) {
        ...
        $valueField = createReadonlyAndHiddenFields(value.title, value.id, 'criteria_value[]');
      } else {
  ...
  if (existingCriteria && existingCriteria.length > 0) {
    ...
    $.each(existingCriteria, function (index, element) {
      ...
      updateMetricsField($newField, element.type, element.metric, element.value, element.condition);
    });
  } else {
...
$(document).on('click', '*[data-toggle="lightbox"]', function (event) {
  ...
  $(event.currentTarget).ekkoLightbox({
    onContentLoaded: function () {
      ...
      $container.parents('.modal-content').css({
        width: 'fit-content'
      });
    }
  });
...
$(function () {
  ...
  const $selectedMenu = $sideBarWrapperScroll.find('li.sidebar-list a[href="' + window.location.pathname + '"]').addClass('sidebar-list-selected');

  // Scroll into view if needed
  if ($selectedMenu.length > 0 && $sideBarWrapperScroll.length > 0 && $selectedMenu.offset().top + $selectedMenu.height() > $sideBarWrapperScroll.height() - $sideBarWrapperScroll[0].scrollTop) {
...
window.XiboInitialise = function (scope, options) {
  ...
  $(scope + ' .XiboHoverButton').on('mouseenter', function (ev) {
  ...
  $(scope + ' input[type="number"][min], ' + scope + ' input[type="number"][max]').each((_idx, input) => {
    const $input = $(input);
    const max = $input.attr('max');
    const min = $input.attr('min');
    $input.on('blur', () => {
      max && $input.val() > max && $input.val(max).trigger('change');
      min && $input.val() < min && $input.val(min).trigger('change');
    });
  });
  ...
  $(scope + ' .localSelect select.form-control').each(function (_idx, el) {
    makeLocalSelect($(el), $(scope).hasClass('modal') ? $(scope) : $('body'));
  });
  ...
  // Colour picker
  $(scope + ' .colorpicker-input:not(.colorpicker-element)').each(function (_i, el) {
    $(el).colorpicker({
      container: $(el).parent()
    });
  });
  ...
  $(scope + ' input[data-role=tagsInputInline], ' + scope + ' input[data-role=tagsInputForm], ' + scope + ' select[multiple][data-role=tagsInputForm]').each(function (_idx, el) {
    const $self = $(el);
    const autoCompleteUrl = $self.data('autoCompleteUrl');
    if (autoCompleteUrl != undefined && autoCompleteUrl != '') {
      // Tags input with autocomplete
  ...
  $(scope + ' .XiboCommand').each(function (_idx, el) {
    ...
    const loadTemplates = function (type) {
      ...
      const inputTemplate = templates.forms.commandInput[type];
      ...
      if (type == 'intent') {
        const inputExtraTemplate = templates.forms.commandInput['intent-extra'];
        ...
        $targetContainer.off('click', '.intent-remove-extra').on('click', '.intent-remove-extra', function (ev) {
          $(ev.currentTarget).parents('.intent-extra-element').remove();
          updateValue(type);
        });
      }
      ...
      if (type == 'http') {
        const inputKeyValueElementTemplate = templates.forms.commandInput['http-key-value'];
        const sectionClasses = ['.query-builder-container', '.http-headers-container', '.http-data-container'];
        const sectionValues = [parsedVal.value != undefined && parsedVal.value.query != undefined ? parsedVal.value.query : null, parsedVal.value != undefined && parsedVal.value.requestOptions != undefined && parsedVal.value.requestOptions.headers != undefined ? parsedVal.value.requestOptions.headers : null, parsedVal.value != undefined && parsedVal.value.requestOptions != undefined && parsedVal.value.requestOptions.body != undefined ? parsedVal.value.requestOptions.body : null];
        ...
        const populateKeyValues = function ($container, values) {
          ...
          for (let i = 0; i < Object.keys(values).length; i++) {
            $container.find('.http-key-value-container').append(inputKeyValueElementTemplate({
              ...{
        ...
        const updateKeyValueRawFields = function (forceUpdateTextArea) {
          ...
          forceUpdateTextArea = forceUpdateTextArea != undefined ? forceUpdateTextArea : false;
          ...
          const createJSONStringFromKeyValue = function ($container) {
            ...
            $container.find('.http-key-value-container .http-key-value-element').each(function (_idx, el) {
              const $el = $(el);
          ...
          const decodeQueryString = function (valueToParse) {
            let parsedValue;
            try {
              parsedValue = '{"' + decodeURI(valueToParse.replace(/&/g, '","').replace(/=/g, '":"')) + '"}';
            } catch (error) {
              console.warn(error);
          ...
          const createQueryStringFromKeyValues = function ($container) {
            ...
            const paramsString = Object.keys(elementsObject).map(key => key + '=' + elementsObject[key]).join('&');
            return paramsString;
          };
          ...
          if ($target.is(':checked') || forceUpdateTextArea) {
            ...
            if (!$parentContainer.find('textarea').hasClass('http-data') || contentType == 'application/json') {
              $parentContainer.find('textarea').val(createJSONStringFromKeyValue($parentContainer));
            } else if (contentType == 'application/x-www-form-urlencoded') {
              $parentContainer.find('textarea').val(createQueryStringFromKeyValues($parentContainer));
            }
          } else {
            ...
            if (!$parentContainer.find('textarea').hasClass('http-data') || contentType == 'application/json') {
              ...
              builtValue = parseJsonFromString($parentContainer.find('textarea').val());
            } else if (contentType == 'application/x-www-form-urlencoded') {
              builtValue = parseJsonFromString(decodeQueryString($parentContainer.find('textarea').val()));
            }
        ...
        for (let i = 0; i < sectionValues.length; i++) {
          ...
          $sectionContainer.find('.http-key-value-add').on('click', function (ev) {
            $(ev.currentTarget).parent().find('.http-key-value-container').append(inputKeyValueElementTemplate({}));
            updateValue(type);
          });

          // Handle Remove extra element
          $sectionContainer.off('click', '.http-key-value-remove').on('click', '.http-key-value-remove', function (ev) {
            $(ev.currentTarget).parents('.http-key-value-element').remove();
            updateValue(type);
          });

          // Handle Raw checkbox input
          $sectionContainer.parent().find('.form-check input[type="checkbox"]').off('change').on('change', function (ev) {
            const $target = $(ev.currentTarget);
            if ($target.hasClass('show-raw-headers') || $target.hasClass('show-raw-data')) {
            ...
            const $parentContainer = $target.parents('.request-section');
            $parentContainer.find($target.data('toggleElement')).toggleClass($target.data('toggleClass'), $target.is(':checked'));
            $parentContainer.find($target.data('toggleElementReverse')).toggleClass($target.data('toggleClass'), !$target.is(':checked'));
          });

          // Value change makes fields to be updated
          $sectionContainer.parent().off('change', '.http-key-value-container .http-key-value-element input').on('change', '.http-key-value-container .http-key-value-element input', function (ev) {
        ...
        $targetContainer.find('.http-contenttype').off('change').on('change', function (ev) {
          const isPlainText = $(ev.currentTarget).val() == 'text/plain';
          $targetContainer.find('.show-raw-data').parent().toggleClass('d-none', isPlainText);
          $targetContainer.find('.show-raw-data').prop('checked', isPlainText).trigger('change');
      ...
      $targetContainer.off('change', 'input:not(.ignore-change), select, textarea').on('change', 'input:not(.ignore-change), select, textarea', function () {
        updateValue(type);
      });
      updateValue(type);
    };
          ...
          switch (valueObj.type) {
            ...
            case 'http':
              ...
              if (requestOptions.headers != undefined) {
                try {
                  requestOptions.headers = SON.parse(requestOptions.headers);
                } catch (error) {
                  console.warn(error);
                ...
                try {
                  if (contentType) {
                    if (contentType == 'application/json') {
                      ...
                      const bodyElements = decodeURI(requestOptions.body).split('&');
    ...
    const updateValue = function (type) {
      ...
      switch (type) {
        ...
        case 'http':
          ...
          if ($container.find('.show-query-builder').is(':checked')) {
            ...
            $container.find('.query-builder-container ' + '.http-key-value-container ' + '.http-key-value-element').each(function (ev) {
            ...
            // Build param string
            const paramsString = Object.keys(paramsObj).map(key => key + '=' + paramsObj[key]).join('&');
        ...
        case 'rs232':
          ...
          $container.find('.rs232-device-name').toggleClass('invalid', deviceNameVal == '');
      ...
      if (invalidValue) {
        $input.val('');
        $mainContainer.find('.command-preview code').text($mainContainer.find('.command-preview').data('invalidMessage'));
    ...
    $(el).find('.command-type').on('change', function (ev) {
      loadTemplates($(ev.currentTarget).val());
    });

    // Link checkbox to input preview
    $(el).find('.show-command-preview').on('change', function (ev) {
      $mainContainer.find('.command-preview').toggle($(ev.currentTarget).is(':checked'));
    });
  ...
  $(scope + ' .XiboData').on('shown.bs.dropdown', '.dropdown-menu-container', function (ev) {
    const $dropdownMenuShown = $(ev.currentTarget).find('.dropdown-menu.show');
    setTimeout(function () {
    ...
    if (url != null && [7, 8].includes(eventTypeId)) {
      $.ajax({
        ...
        if (response.data[0]?.fullScreenCampaignId) {
          $form.find('#fullScreenCampaignId').val(response.data[0].fullScreenCampaignId).trigger('change');
        }
  ...
  $(scope + ' .full-screen-layout-form').on('click autoOpen', function (ev) {
    ...
    const playlistId = $target.closest('form').find('#fullScreen-playlistId').val();
    ...
    if ($('#full-screen-schedule-modal').length === 0) {
      ...
      $modal.on('show.bs.modal', function () {
        ...
        $form.find('.pagedSelect select.form-control').each(function (_idx, el) {
          const $target2 = $(el);
          if ($target2.attr('name') == 'mediaId' && mediaId != null) {
            $target2.data('initialValue', mediaId);
          } else if ($target2.attr('name') == 'playlistId' && playlistId != null) {
        ...
        // init color picker
        $form.find('.colorpicker-input:not(.colorpicker-element)').each(function (_idx, el) {
          $(el).colorpicker({
            container: $(el).parent()
          });
        });
        ...
        if (eventTypeId == 7) {
          $resolutionControl.children('div').children('small.form-text.text-muted').text($resolutionSelect.data('transMediaHelpText'));
        } else if (eventTypeId == 8) {
          $resolutionControl.children('div').children('small.form-text.text-muted').text($resolutionSelect.data('transPlaylistHelpText'));
        }
  ...
  if (forms && typeof forms.initFields === 'function') {
    ...
    forms.initFields(scope === ' ' ? 'body' : scope, null, options && options.targetId ? options.targetId : null, options && options.readOnlyMode ? options.readOnlyMode : false);
  }
};
 ...
 * @param {Object} data
 */
window.XiboFormRender = function (sourceObj) {
    ...
    success: function (response) {
      ...
      if (response.success) {
        ...
        if (response.buttons !== '') {
          ...
          $.each(response.buttons, function (index, value) {
            i++;
            const extrabutton = $('<button id="dialog_btn_' + i + '" class="btn">').html(index);
            if (i === count) {
        ...
        $('a[data-toggle="tab"]', dialog).each(function (_idx, el) {
          if ($(el).data().enlarge === 1 && $(el).closest('li').hasClass('active')) {
...
window.formRenderDetectSpacingIssues = function (element) {
  ...
  if (value !== '' && (value.startsWith(' ') || value.endsWith(' ') || value.indexOf('  ') > -1)) {
    ...
    const warning = $('<span></span>').addClass('fa fa-exclamation-circle spacing-warning-icon').attr('title', translations.spacesWarning);
...
window.XiboMultiSelectFormRender = function (button) {
  ...
  if (matches.length > 0) {
    ...
    extrabutton.on('click', function (ev) {
      $(ev.currentTarget).append(' <span class="saving fa fa-cog fa-spin"></span>');
...
window.XiboSubmitResponse = function (response, form) {
  ...
  if (response.success) {
    ...
    if (!apply) {
      ...
      if ($(form).data('nextFormUrl') !== undefined) {
        const responseId = $(form).data('nextFormIdProperty') === undefined ? response.id : response.data[$(form).data('nextFormIdProperty')];
 ...
 * @param addRandomId
 */
window.makePagedSelect = function (element, parent, dataFormatter) {
  let addRandomId = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : false;
  ...
  if ([undefined, ''].indexOf(element.data('initialValue')) == -1 && [undefined, ''].indexOf(element.data('initialKey')) == -1) {
    ...
    $.ajax({
      ...
      data.data.forEach(object => {
        ...
        if (isSelected) {
          const option = new Option(object[textProperty], object[idProperty], isSelected, isSelected);
 ...
 * @param addRandomId
 */
window.makeLocalSelect = function (element, parent) {
  let addRandomId = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;
  // If we need to append random id
  if (addRandomId === true) {
    const randomNum = Math.floor(1000000000 + Math.random() * 9000000000);
    const previousId = $(element).attr('id');
  ...
  element.select2({
    ...
    matcher: function (params, data) {
      const testElementFilter = function (filter, elementFilterClassName) {
        const elementFilterClass = $(data.element).data()[elementFilterClassName];

        // Get element class array ( one or more elements split by comma)
        const elementClassArray = elementFilterClass != undefined ? elementFilterClass.replace(' ', '').split(',') : [];
      ...
      queryText = queryText.trim().split(',').map(subStr => subStr.trim()).filter(subStr => subStr !== '');
...
window.userPreferencesFormSubmit = function () {
  ...
  $form.find('input[type="checkbox"]').each(function (_idx, el) {
    ...
    // Create hidden input
    $('<input type="hidden">').attr('id', id).attr('name', id).val(value).appendTo($(el).parent());
...
window.initDatePicker = function ($element, baseFormat, displayFormat, options, onChangeCallback, clearButtonActive, onClearCallback) {
  // Default values
  ...
  if (clearButtonActive) {
    $inputElement.parents('.input-group').find('.date-clear-button').removeClass('d-none').on('click', function () {
...
window.createMiniLayoutPreview = function (previewUrl) {
  ...
  const previewTemplate = Handlebars.compile('<iframe scrolling="no" src="{{url}}" width="{{width}}px" ' + 'height="{{height}}px" style="border:0;"></iframe>');
  ...
  $layoutPreview.find('#sizeBtn').off().on('click', function (ev) {
    ...
    $(ev.currentTarget).toggleClass('fa-arrow-circle-down', $layoutPreview.hasClass('large'));
    $(ev.currentTarget).toggleClass('fa-arrow-circle-up', !$layoutPreview.hasClass('large'));
    // Re-show play button
...
window.moveFolderMultiSelectFormOpen = function (dialog) {
  ...
  const folderContainer = '<div class="card p-3 mb-3 bg-light" ' + 'id="container-folder-form-tree"></div>';
  const $hiddenInput = $('<input name="folderId" type="hidden" id="formFolderId">');
  $hiddenInput.on('change', function (ev) {
    dialog.data().commitData = {
      folderId: $(ev.currentTarget).val()
    };
  });
...
window.triggerWebhookMultiSelectFormOpen = function (dialog) {
  const $triggerCode = $('<div class="form-group row">' + '<label class="col-sm-2 control-label" for="triggerCode" ' + 'accesskey="">' + translations.triggerCodeLabel + '</label>' + '<div class="col-sm-10">' + '<input class="form-control" name="triggerCode" type="text" ' + 'id="triggerCode" value="">' + '<span class="help-block">' + translations.triggerCodeHelpText + '</span>' + '</div>' + '</div>');
  ...
  $(scope + ' .XiboGrid').each(function (i, el) {
    ...
    if (gridName != undefined) {
      ...
      $.each(Object.values(formValuesUpdated), function (key, element) {
        ...
        try {
          const field = form.find('input[name="' + fieldName + '"],' + ' select[name="' + fieldName + '"],' + ' select[name="' + element.name + '"]');
    ...
    const filterRefresh = _.debounce(function () {
      ...
      $target.closest('.XiboGrid').find('table.dataTable').first().DataTable().ajax.reload();
    }, 500);
    ...
    if ($target.find('.XiboFilter .nav-tabs').length > 0) {
      if ($target.find('.XiboFilter .nav-tabs .clear-filter-btn-container').length === 0 && form.length > 0) {
        $target.find('.XiboFilter .nav-tabs').append(buttonTemplate);
      }
    } else {
      if ($target.find('.XiboFilter .clear-filter-btn-container').length === 0 && form.length > 0) {
    ...
    // Handle clear filter
    $target.find('.XiboFilter .clear-filter-btn').off().on('click', function () {
      // Reset fields
      form[0].reset();
    ...
    $target.find('.XiboFilter form input[type="number"]').on('change', filterRefresh);
    $target.find('.XiboFilter form input[type="checkbox"]').on('change', filterRefresh);
    $target.find('.XiboFilter form select').on('change', filterRefresh);
    $target.find('.XiboFilter form input.dateControl').on('change', filterRefresh);
    ...
    $target.find('.XiboFilter form input[data-role="tagsInputInline"]').on('change', filterRefresh);

    // Preview layout buttons
    $target.on('click', '#dropdown-menu-right-container ' + 'a.dropdown-item[data-custom-handler="createMiniLayoutPreview"]', function (e) {
    ...
    const gridFolderState = rememberFolderTreeStateGlobally ? 'grid-folder-tree-state' : 'grid_' + gridName;
    // init the jsTree
    initJsTreeAjax($target.find('#container-folder-tree'), gridFolderState, false);
  });
};
...
window.dataTableDraw = function (e, settings, callBack) {
  ...
  const $folderController = target.closest('.XiboGrid').find('.folder-controller');
  if (enabledButtons.length > 0) {
    ...
    $(enabledButtons).each(function (_idx, el) {
      ...
      if (!searchByKey(buttons, 'id', $button.data('id'))) {
        buttons.push({
          ...
          sortGroup: $button.data('sortGroup') != undefined ? $button.data('sortGroup') : 0
        });
      }
    });
    ...
    const output = templates.dataTable.multiSelectButton({
    ...
    // Bind to our output
    target.closest('.dataTables_wrapper').find('.dataTables_info a.XiboMultiSelectFormButton').on('click', function (ev) {
      const $target = $(ev.currentTarget);
      if ($target.data('customHandler') != undefined && typeof window[$target.data('customHandler')] == 'function') {
        window[$target.data('customHandler')](ev.currentTarget);
      } else {
        XiboMultiSelectFormRender(ev.currentTarget);
      }
    });
    target.closest('.dataTables_wrapper').find('.dataTables_info a.XiboMultiSelectFormCustomButton').on('click', function (ev) {
      window[$(ev.currentTarget).data('customHandler')](ev.currentTarget);
    });

    // Bind click to select all button
    target.closest('.dataTables_wrapper').find('.dataTables_info button.select-all').on('click', function () {
...
window.dataTableCreateTagEvents = function (e, settings) {
  ...
  table.on('click', '.btn-tag', function (ev) {
    ...
    if (tableId == 'playlistLibraryMedia') {
      form.find('#filterMediaTag').tagsinput('add', tagText, {
        allowDuplicates: false
      });
    } else if (tableId == 'displayGroupDisplays') {
      form.find('#dynamicCriteriaTags').tagsinput('add', tagText, {
        allowDuplicates: false
      });
    } else {
      // Add text to form
      form.find('#tags').tagsinput('add', tagText, {
        allowDuplicates: false
      });
    }
...
window.dataTableStateLoadCallback = function (settings, callback) {
  const statePreferenceName = $('#' + settings.sTableId).data().statePreferenceName;
  const option = statePreferenceName !== undefined ? statePreferenceName : settings.sTableId + 'Grid';
  let data = {};
  $.ajax({
    type: 'GET',
    async: false,
...
window.XiboMultiSelectPermissionsFormOpen = function (button) {
  ...
  if ($matches.length == 0) {
    ...
    XiboFormRender(requestUrl, {
      ids: matchIds.toString()
    });
  }
};
...
window.adjustDatatableSize = function (reload) {
  ...
  $('#grid-folder-filter').toggle('fast', function () {
    ...
    if (reload) {
      $('#grid-folder-filter').closest('.XiboGrid').find('table.dataTable').DataTable().ajax.reload();
    }
...
/* Folders */
window.initJsTreeAjax = function (container, id, isForm, ttl) {
  let onReady = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : null;
  let onSelected = arguments.length > 5 && arguments[5] !== undefined ? arguments[5] : null;
  let onBuildContextMenu = arguments.length > 6 && arguments[6] !== undefined ? arguments[6] : null;
  let plugins = arguments.length > 7 && arguments[7] !== undefined ? arguments[7] : [];
  let homeFolderId = arguments.length > 8 && arguments[8] !== undefined ? arguments[8] : null;
  // Default values
  ...
  if ($(container).length) {
    ...
    $(container).jstree({
              ...
              if (isForm === false && buttonPermissions.share) {
                items['Share'] = {
                  ...
                  action: function (obj) {
                    ...
                    const selectedNodes = $(container).jstree('get_selected', true).map(el => {
                      return Number(el.id);
                    });
                    if (selectedNodes.length > 1) {
                      // If we have more than one selected node
                      // call share multiple
                      XiboFormRender(permissionsUrl.replace(':entity', 'multiple/form/Folder'), {
      ...
      if (localStorage.getItem('hideFolderTree') !== undefined && localStorage.getItem('hideFolderTree') !== null && JSON.parse(localStorage.getItem('hideFolderTree')) !== $('#grid-folder-filter').is(':hidden')) {
      ...
      if (isForm) {
        ...
        const selectedFolder = !$(folderIdInputSelector).val() ? homeNodeId : $(folderIdInputSelector).val();
        if (selectedFolder !== undefined && selectedFolder !== '') {
          // eslint-disable-next-line no-invalid-this
          const self = this;
          $(self).jstree('select_node', selectedFolder);
          if ($('#originalFormFolder').length) {
            $('#originalFormFolder').text($(self).jstree().get_path($(self).jstree('get_selected', true)[0], ' > '));
          }
          if ($('#selectedFormFolder').length && folderIdInputSelector === '#formFolderId') {
            $('#selectedFormFolder').text($(self).jstree().get_path($(self).jstree('get_selected', true)[0], ' > '));
          }
        }
      }
      ...
      let folderIdInputSelector = isForm ? '#' + id + ' #folderId' : '#folderId';
    ...
    $('.btnCloseInnerModal').on('click', function (e) {
      e.preventDefault();
      const folderTreeModalId = isForm ? '#folder-tree-form-modal' : '#folder-tree-modal';
...
module.exports = {
  ...
  /**
     ...
     */
  reloadTooltips: function (container) {
    let {
      forcedOption = null,
      placement = 'auto'
    } = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
    // Use global var or option
    const enableTooltips = forcedOption != null ? forcedOption : this.displayTooltips;
    const tooltipSelector = enableTooltips ? '[data-toggle="tooltip"]:not(:disabled)' : '[data-toggle="tooltip"].tooltip-always-on:not(:disabled)';
  ...
  timeFormat: function (timeInSeconds) {
    let alwaysShowMinutes = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : true;
    ...
    const mDisplay = m > 0 || hDisplay != '' || alwaysShowMinutes ? zeroBefore(m) + ':' : '';
  ...
  formatFileSize: function (value) {
    return (b = Math, c = b.log, d = 1e3, e = c(value) / c(d) | 0, value / b.pow(d, e)).toFixed(2) + ' ' + (e ? 'kMGTPEZY'[--e] + 'B' : 'Bytes');
  },
  /**
  ...
  hasTarget: function (object, targetType) {
    ...
    if (targetData.indexOf('all') !== -1) {
      return true;
    } else if (targetData.indexOf(targetType) !== -1) {
      return true;
    }
    return false;
  },
  ...
  clearContainer: function ($container) {
    ...
    // Select2
    $container.find('select[data-select2-id]').select2('destroy');
  ...
  handleEditorMinimumDimensions: function (editor, forceReload) {
    ...
    const updateEditor = function () {
      ...
      editor.showMinDimensionsMessage = currentWidth < minWindowWidth || currentHeight < minWindowHeight;
      ...
      if (editorInitalState != editor.showMinDimensionsMessage || forceReload) {
        ...
        if (editor.showMinDimensionsMessage && !minSizeWarningOff) {
          ...
          $customOverlay.removeClass('custom-overlay').addClass('custom-overlay-clone min-res-overlay');
          $customOverlay.appendTo(editor.editorContainer);

          // Click X button to dismiss and save preference
          editor.editorContainer.find('.min-res-message .close-res-message-button').on('click', function () {
    ...
    $(window).on('resize.' + editor.mainObjectType, _.debounce(updateEditor, resizeThrottle));
...
const PlayerHelper = function () {
  ...
  this.getPinnedSlots = function (dataSlots) {
    return Object.keys(dataSlots).reduce(function (a, b) {
  ...
  this.getItemsByKey = (items, itemsKey, isStandalone) => {
    if (isStandalone && items.hasOwnProperty(itemsKey) && Object.keys(items[itemsKey]).length > 0) {
  ...
  this.getMinAndMaxSlot = function (collection) {
    ...
    const minSlot = collection === null ? minValue : Math.min(...getSlots(collection));
    const maxSlot = collection === null ? minValue : Math.max(...getSlots(collection));
    return {
      minSlot,
      maxSlot
    };
  ...
  this.getMaxMinSlot = (objectsArray, itemsKey, isStandalone) => {
    const minValue = 1;
    const groupItems = objectsArray?.length > 0 ? objectsArray.reduce((a, b) => {
      return [...a, ...self.getItemsByKey(b, itemsKey, isStandalone)];
    }, []) : null;
    const getSlots = items => items.map(function (elem) {
      return elem?.slot || 0;
    });
    const minSlot = groupItems === null ? minValue : Math.min(...getSlots(groupItems)) + 1;
    const maxSlot = groupItems === null ? minValue : Math.max(...getSlots(groupItems)) + 1;
    return {
      minSlot,
      maxSlot
    };
  ...
  this.isMarquee = function (effect) {
    return effect === 'marqueeLeft' || effect === 'marqueeRight' || effect === 'marqueeUp' || effect === 'marqueeDown';
  };
  this.renderElement = function (hbs, props, isStatic) {
    ...
    const $renderedElem = $(hbsTemplate).first().attr('id', props.elementId).addClass(`${props.uniqueID}--item`).css(cssStyles);
    if (!props.isGroup && props.dataOverride === 'text' && props.group && props.group.isMarquee && (props.effect === 'marqueeLeft' || props.effect === 'marqueeRight')) {
      $renderedElem.get(0).style.removeProperty('white-space');
      $renderedElem.get(0).style.setProperty('white-space', props?.textWrap ? 'unset' : 'nowrap', 'important');
    }
    return $renderedElem.prop('outerHTML');
  };
  this.renderDataItem = function (isGroup, dataItemKey, dataItem, item, slot, maxSlot, isPinSlot, pinnedSlots, groupId, $groupContent, groupObj, meta, $content) {
    ...
    if (item.hasOwnProperty('hbs') && typeof item.hbs === 'function' && dataItemKey !== 'empty') {
      let groupItemStyles = {
      ...
      if ($groupContent && $groupContent.find(groupKey.replace('%key%', dataItemKey)).length === 0) {
      ...
      if (!isGroup && item.dataOverride === 'text' && groupObj.isMarquee) {
        if (item.effect === 'marqueeLeft' || item.effect === 'marqueeRight') {
          if ($groupContent.find(groupKey.replace('%key%', dataItemKey)).length === 1) {
            $groupContent.find(groupKey.replace('%key%', dataItemKey)).remove();
          }
          isSingleElement = true;
        } else if (item.effect === 'marqueeDown' || item.effect === 'marqueeUp') {
          isSingleElement = false;
        }
      }
      const $itemContainer = isSingleElement ? $groupContent : $groupContent.find(groupKey.replace('%key%', dataItemKey));
      const props = Object.assign(item.templateData, {
        isGroup
      }, String(item.dataOverride).length > 0 && String(item.dataOverrideWith).length > 0 ? dataItem : {
        data: dataItem
      }, {
        group: groupObj
      });
      ...
      if (props.isExtended) {
        if (props.type === 'dataset' && props.hasOwnProperty('datasetField') && dataItem.hasOwnProperty(props.datasetField)) {
          props[props.dataOverride] = dataItem[props.datasetField];
        } else {
          const extendWith = transformer.getExtendedDataKey(props.dataOverrideWith);
          if (props.dataOverride === extendWith && dataItem.hasOwnProperty(extendWith)) {
            props[props.dataOverride] = dataItem[extendWith];
          }
        }
      }
      const $elementContent = $(self.renderElement(item.hbs, props));
      ...
      item.onTemplateRender() !== undefined && item.onTemplateRender()(item.elementId, $itemContainer.find(`.${itemID}--item`), $content.find(`.${itemID}--item`), {
        item,
        ...item.templateData,
        data: dataItem
      }, meta);
      ...
      const $itemContainer = $groupContent.find(groupKey.replace('%key%', dataItemKey));
...
/***/ "./node_modules/css-loader/dist/cjs.js!./node_modules/postcss-loader/dist/cjs.js??ruleSet[1].rules[3].use[2]!./node_modules/sass-loader/dist/cjs.js!./ui/src/style/forms.scss":
/*!************************************************************************************************************************************************************************************!*\
  !*** ./node_modules/css-loader/dist/cjs.js!./node_modules/postcss-loader/dist/cjs.js??ruleSet[1].rules[3].use[2]!./node_modules/sass-loader/dist/cjs.js!./ui/src/style/forms.scss ***!
  \************************************************************************************************************************************************************************************/
...
// Module
___CSS_LOADER_EXPORT___.push([module.id, "/*!\n * Copyright (C) 2023 Xibo Signage Ltd\n *\n * Xibo - Digital Signage - https://xibosignage.com\n *\n * This file is part of Xibo.\n *\n * Xibo is free software: you can redistribute it and/or modify\n * it under the terms of the GNU Affero General Public License as published by\n * the Free Software Foundation, either version 3 of the License, or\n * any later version.\n *\n * Xibo is distributed in the hope that it will be useful,\n * but WITHOUT ANY WARRANTY; without even the implied warranty of\n * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the\n * GNU Affero General Public License for more details.\n *\n * You should have received a copy of the GNU Affero General Public License\n * along with Xibo.  If not, see <http://www.gnu.org/licenses/>.\n */\n.input-info-container {\n  float: right;\n  padding-top: 6px;\n}\n.input-info-container .input-info {\n  float: right;\n  margin-left: 4px;\n  color: #74ACFA;\n}\n.input-info-container .input-info.checkbox-popup {\n  padding-top: 3px;\n}\n\n.text-area-buttons {\n  display: flex;\n  justify-content: flex-end;\n  background-color: #E8F1FE;\n  border-radius: 0.25rem 0.25rem 0 0;\n}\n\n/* Code input */\n.code-input-editor-container {\n  border: 2px solid #CCC;\n  border-radius: 0.25rem;\n  position: relative;\n  overflow-y: auto;\n}\n.code-input-editor-container.non-scroll {\n  overflow-y: unset;\n}\n.code-input-editor-container .code-input-editor-scrollable {\n  overflow-y: auto;\n  min-height: 160px;\n  max-height: 300px;\n}\n.code-input-editor-container .code-input-fs-btn {\n  display: none;\n  position: absolute;\n  right: 6px;\n  top: 6px;\n}\n.code-input-editor-container .code-input-fs-btn .hide-on-code-fs {\n  display: block;\n}\n.code-input-editor-container .code-input-fs-btn .show-on-code-fs {\n  display: none;\n}\n.code-input-editor-container .code-input-editor {\n  width: 100%;\n  height: 100%;\n}\n.code-input-editor-container .code-input-editor .cm-focused {\n  outline: none !important;\n}\n.code-input-editor-container .cm-gutters {\n  display: none;\n}\n.code-input-editor-container:hover .code-input-fs-btn {\n  display: block;\n  opacity: 0.4;\n}\n.code-input-editor-container:hover .code-input-fs-btn:hover {\n  opacity: 0.8;\n}\n.code-input-editor-container.code-input-fs {\n  position: fixed;\n  width: 60vw;\n  height: 60vh;\n  left: 20vw;\n  top: 20vh;\n  z-index: 2040;\n  background-color: #fff;\n}\n.code-input-editor-container.code-input-fs .code-input-editor-scrollable {\n  max-height: 100%;\n}\n.code-input-editor-container.code-input-fs .code-input-fs-btn {\n  display: block;\n}\n.code-input-editor-container.code-input-fs .hide-on-code-fs {\n  display: none;\n}\n.code-input-editor-container.code-input-fs .show-on-code-fs {\n  display: block;\n}\n\n.code-fs-placeholder {\n  border: 2px solid #CCC;\n  border-radius: 0.25rem;\n  background-color: #E9E9E9;\n  min-height: 160px;\n  max-height: 400px;\n}\n\n.code-fs-overlay {\n  z-index: 2030 !important;\n}\n\n/* Data input */\n.date-input-group .input-group > .form-control:not([type=hidden]) {\n  border-top-left-radius: 0.25rem;\n  border-bottom-left-radius: 0.25rem;\n}\n.date-input-group .input-group > .form-control:not(:disabled) {\n  background-color: white;\n  cursor: pointer;\n}\n\n/* Header */\n.header {\n  font-weight: bold;\n  font-size: 1.1rem;\n  color: #1775F6;\n  margin-left: -4px;\n}\n.header.main {\n  color: #0E4694;\n  font-size: 1.3rem;\n}\n\n/* Rich text input */\n.rich-text-temp-container {\n  background-color: #000000;\n}\n\n#richTextDetachedOverlay {\n  z-index: 1049 !important;\n}\n\n.ck-body-wrapper .ck-balloon-panel {\n  z-index: 1051 !important;\n}\n\n.ck-editor-body-detached .ck-body-wrapper .ck-balloon-panel[data-main-toolbar] {\n  z-index: 1052 !important;\n  right: 150px !important;\n  left: auto !important;\n  top: auto !important;\n  bottom: calc(100vh - 100px) !important;\n  width: calc(100vw - 300px) !important;\n  display: block;\n}\n.ck-editor-body-detached .ck-body-wrapper .ck-balloon-panel[data-main-toolbar]::before, .ck-editor-body-detached .ck-body-wrapper .ck-balloon-panel[data-main-toolbar]:after {\n  display: none;\n}\n\n.ck-editor-body-detached .ck-body-wrapper .ck-balloon-panel:not(.ck-powered-by-balloon):not([data-main-toolbar]) {\n  top: 148px !important;\n  left: 160px !important;\n  opacity: 0.95;\n}\n\n.ck-editor-body-detached .ck-body-wrapper .ck-balloon-panel.ck-powered-by-balloon {\n  top: auto !important;\n  left: auto !important;\n  bottom: 16px !important;\n  right: 16px !important;\n  display: block;\n}\n\n.rich-text-main-container {\n  width: 100%;\n  transform-origin: top right;\n  box-shadow: none !important;\n  overflow: hidden;\n  border-radius: 0 0 0.25rem 0.25rem;\n}\n.rich-text-main-container textarea {\n  display: none;\n}\n.rich-text-main-container .rich-text-container-wrapper {\n  font-size: initial;\n  max-height: 202px;\n  overflow: auto;\n}\n.rich-text-main-container .rich-text-container-wrapper.loading > * {\n  display: none;\n}\n.rich-text-main-container .rich-text-container, .rich-text-main-container .rich-text-editor {\n  overflow: visible;\n}\n.rich-text-main-container .text-area-buttons {\n  width: 100%;\n  position: sticky;\n  left: 0;\n  top: 0;\n  z-index: 1;\n}\n.rich-text-main-container .text-area-buttons button:focus {\n  box-shadow: none !important;\n}\n.rich-text-main-container .ck-editor__editable_inline {\n  outline-color: #EB7857;\n  outline-style: solid;\n  border-radius: 0 !important;\n}\n.rich-text-main-container.source .rich-text-container-wrapper {\n  background-color: #fff !important;\n  overflow: initial;\n}\n.rich-text-main-container.source .rich-text-container-wrapper .rich-text-container {\n  height: 100% !important;\n}\n.rich-text-main-container.source .ck-editor__editable_inline {\n  display: none;\n}\n.rich-text-main-container.source textarea {\n  display: initial !important;\n  opacity: initial !important;\n  height: 100% !important;\n  min-height: 160px;\n}\n.rich-text-main-container.source .rich-text-editor {\n  display: none;\n}\n.rich-text-main-container.source .viewSourceButton {\n  outline: 2px solid #898989;\n  outline-offset: -5px;\n  border-radius: 8px;\n}\n.rich-text-main-container.source .zoomButton {\n  display: none;\n}\n.rich-text-main-container.detached {\n  z-index: 1050;\n  position: fixed;\n  top: 100px;\n  right: 150px;\n  outline-width: 1px;\n  width: calc(100vw - 300px) !important;\n  height: calc(100vh - 200px) !important;\n  overflow: hidden;\n}\n.rich-text-main-container.detached .rich-text-container-wrapper {\n  height: calc(100% - 28.5px) !important;\n  max-height: none;\n}\n.rich-text-main-container.detached .rich-text-container {\n  height: 100%;\n}\n.rich-text-main-container.detached .rich-text-dimensions-control {\n  background: #FCFCFC;\n  color: #333333;\n}\n.rich-text-main-container.detached .rich-text-dimensions-control .control-label {\n  display: inline-block;\n}\n.rich-text-main-container.detached .text-area-buttons {\n  border-radius: 0.25rem 0.25rem 0 0;\n  padding: 6px;\n  background-color: #333333;\n  background-color: rgba(51, 51, 51, 0.9);\n}\n.rich-text-main-container.detached .text-area-buttons button {\n  color: #fff;\n}\n\n.rich-text-dimensions-control {\n  padding: 6px 8px;\n  background: #0E4694;\n  margin: 0;\n  display: flex;\n  gap: 8px;\n  align-items: center;\n  justify-content: flex-end;\n}\n.rich-text-dimensions-control .control-label {\n  display: none;\n}\n.rich-text-dimensions-control .form-group {\n  margin: 0;\n}\n.rich-text-dimensions-control .form-group input {\n  height: 25px;\n  width: 100%;\n}\n.rich-text-dimensions-control .form-group input[type=checkbox] {\n  margin: 0 6px;\n}\n.rich-text-dimensions-control select {\n  padding: 4px;\n  height: 28px;\n}\n\n.dataset-column-selector, .ticker-tag-selector, .dataset-column-style-selector {\n  display: flex;\n  gap: 6px;\n}\n.dataset-column-selector .connectedlist, .ticker-tag-selector .connectedlist, .dataset-column-style-selector .connectedlist {\n  flex: 1;\n  min-width: 0;\n}\n.dataset-column-selector .connectedlist .text-info, .ticker-tag-selector .connectedlist .text-info, .dataset-column-style-selector .connectedlist .text-info {\n  color: #121A5E !important;\n  font-weight: bold;\n}\n.dataset-column-selector .connectedlist .ui-sortable, .ticker-tag-selector .connectedlist .ui-sortable, .dataset-column-style-selector .connectedlist .ui-sortable {\n  width: 100% !important;\n  background-color: #74ACFA !important;\n}\n.dataset-column-selector .connectedlist .li-sortable, .ticker-tag-selector .connectedlist .li-sortable, .dataset-column-style-selector .connectedlist .li-sortable {\n  width: calc(100% - 12px);\n}\n\n.ticker-tag-selector, .dataset-column-style-selector {\n  flex-wrap: wrap;\n}\n.ticker-tag-selector .ticker-tag-styles, .ticker-tag-selector .dataset-column-styles, .dataset-column-style-selector .ticker-tag-styles, .dataset-column-style-selector .dataset-column-styles {\n  width: 100%;\n}\n.ticker-tag-selector .ticker-tag-styles .ticker-tag-style-title, .ticker-tag-selector .ticker-tag-styles .dataset-col-style-title, .ticker-tag-selector .dataset-column-styles .ticker-tag-style-title, .ticker-tag-selector .dataset-column-styles .dataset-col-style-title, .dataset-column-style-selector .ticker-tag-styles .ticker-tag-style-title, .dataset-column-style-selector .ticker-tag-styles .dataset-col-style-title, .dataset-column-style-selector .dataset-column-styles .ticker-tag-style-title, .dataset-column-style-selector .dataset-column-styles .dataset-col-style-title {\n  writing-mode: vertical-lr;\n  font-size: 1.2rem;\n  color: #121A5E;\n}\n.ticker-tag-selector .ticker-tag-styles .ticker-tag-style, .ticker-tag-selector .ticker-tag-styles .dataset-col-style, .ticker-tag-selector .dataset-column-styles .ticker-tag-style, .ticker-tag-selector .dataset-column-styles .dataset-col-style, .dataset-column-style-selector .ticker-tag-styles .ticker-tag-style, .dataset-column-style-selector .ticker-tag-styles .dataset-col-style, .dataset-column-style-selector .dataset-column-styles .ticker-tag-style, .dataset-column-style-selector .dataset-column-styles .dataset-col-style {\n  display: flex;\n}\n.ticker-tag-selector .ticker-tag-styles .ticker-tag-style-properties, .ticker-tag-selector .ticker-tag-styles .dataset-col-style-properties, .ticker-tag-selector .dataset-column-styles .ticker-tag-style-properties, .ticker-tag-selector .dataset-column-styles .dataset-col-style-properties, .dataset-column-style-selector .ticker-tag-styles .ticker-tag-style-properties, .dataset-column-style-selector .ticker-tag-styles .dataset-col-style-properties, .dataset-column-style-selector .dataset-column-styles .ticker-tag-style-properties, .dataset-column-style-selector .dataset-column-styles .dataset-col-style-properties {\n  border-left: 6px solid #74ACFA;\n  padding-left: 12px;\n  margin-left: 6px;\n}\n.ticker-tag-selector .ticker-tag-styles .ticker-tag-style-properties .form-group, .ticker-tag-selector .ticker-tag-styles .dataset-col-style-properties .form-group, .ticker-tag-selector .dataset-column-styles .ticker-tag-style-properties .form-group, .ticker-tag-selector .dataset-column-styles .dataset-col-style-properties .form-group, .dataset-column-style-selector .ticker-tag-styles .ticker-tag-style-properties .form-group, .dataset-column-style-selector .ticker-tag-styles .dataset-col-style-properties .form-group, .dataset-column-style-selector .dataset-column-styles .ticker-tag-style-properties .form-group, .dataset-column-style-selector .dataset-column-styles .dataset-col-style-properties .form-group {\n  margin-bottom: 0.5rem;\n}\n\n.xibo-command-builder .command-type {\n  display: block;\n  width: 100%;\n}\n.xibo-command-builder .command-inputs {\n  border-radius: 0 0 6px 6px;\n  padding: 12px 6px;\n  margin: 0px 1px 4px 1px;\n  background-color: #E8F1FE;\n}\n.xibo-command-builder .command-inputs .col-12 {\n  max-width: 100%;\n}\n.xibo-command-builder .command-inputs .intent-name {\n  width: 100%;\n  margin: 6px 0;\n}\n.xibo-command-builder .command-inputs .intent-type-container {\n  font-size: 0;\n}\n.xibo-command-builder .command-inputs .intent-type-container .intent-type-label {\n  width: 30%;\n  font-size: 14px;\n}\n.xibo-command-builder .command-inputs .intent-type-container .intent-type {\n  width: 70%;\n}\n.xibo-command-builder .command-inputs .intent-extra-element {\n  display: flex;\n  gap: 2px;\n  margin-bottom: 2px;\n}\n.xibo-command-builder .command-inputs .intent-extra-element select {\n  padding: 0;\n  width: 30%;\n}\n.xibo-command-builder .command-preview {\n  border-radius: 6px;\n  padding: 8px !important;\n}\n\n.playlist-mixer .mixer-playlist-container {\n  padding: 0.4rem 0.2rem 0 0.2rem !important;\n}\n.playlist-mixer .subplaylist-item-row > *, .playlist-mixer .subplaylist-items-header > * {\n  margin-right: 2px;\n}\n.playlist-mixer .subplaylist-item-row .subplaylist-id-container-header, .playlist-mixer .subplaylist-items-header .subplaylist-id-container-header {\n  margin-left: 70px;\n}\n.playlist-mixer .subplaylist-item-row .subplaylist-spots-container, .playlist-mixer .subplaylist-item-row .subplaylist-spots-length-container, .playlist-mixer .subplaylist-items-header .subplaylist-spots-container, .playlist-mixer .subplaylist-items-header .subplaylist-spots-length-container {\n  width: 100px;\n}\n.playlist-mixer .subplaylist-item-row .subplaylist-spots-container input, .playlist-mixer .subplaylist-item-row .subplaylist-spots-length-container input, .playlist-mixer .subplaylist-items-header .subplaylist-spots-container input, .playlist-mixer .subplaylist-items-header .subplaylist-spots-length-container input {\n  width: 100%;\n}\n.playlist-mixer .subplaylist-item-row .subplaylist-id-container, .playlist-mixer .subplaylist-items-header .subplaylist-id-container {\n  width: 100px;\n}\n.playlist-mixer .subplaylist-item-row .subplaylist-id-container .select2-container, .playlist-mixer .subplaylist-items-header .subplaylist-id-container .select2-container {\n  min-width: 100% !important;\n}\n.playlist-mixer .subplaylist-item-row .subplaylist-spots-fill-container, .playlist-mixer .subplaylist-items-header .subplaylist-spots-fill-container {\n  width: 120px;\n}\n.playlist-mixer .subplaylist-item-row .subplaylist-spots-fill-container .select2-container, .playlist-mixer .subplaylist-items-header .subplaylist-spots-fill-container .select2-container {\n  min-width: 100% !important;\n}\n.playlist-mixer .subplaylist-items-footer {\n  position: sticky;\n  left: 0;\n  bottom: 0;\n  padding: 10px;\n  width: 100%;\n  text-align: center !important;\n}\n\n.canvas-widget-control {\n  padding: 8px;\n  background-color: #1775F6;\n}\n.canvas-widget-control .canvas-widget-control-dropdown {\n  color: #E8F1FE;\n  margin: 0;\n}\n.canvas-widget-control .canvas-widget-control-dropdown .input-info-container {\n  display: inline-block;\n  float: none;\n  padding: 0;\n}\n.canvas-widget-control .canvas-widget-control-dropdown .canvas-widget-control-add {\n  cursor: pointer;\n  float: right;\n  padding: 0 6px;\n  border-radius: 4px;\n}\n.canvas-widget-control .canvas-widget-control-dropdown .canvas-widget-control-add i {\n  margin-right: 4px;\n}\n.canvas-widget-control .canvas-widget-control-dropdown .canvas-widget-control-add:hover {\n  background-color: #085dd2;\n}\n\n.widget-info-container {\n  text-align: center;\n  padding: 6px;\n  background-color: #74ACFA;\n  color: #F3F8FF;\n  white-space: nowrap;\n  overflow: hidden;\n  text-overflow: ellipsis;\n}\n.widget-info-container .widget-info-name {\n  font-weight: bold;\n}\n\n.snippet-selector {\n  margin-top: -12px;\n}\n.snippet-selector .select2-selection {\n  background-color: #E8F1FE !important;\n}\n.snippet-selector .select2-selection__placeholder {\n  color: #121A5E !important;\n  font-weight: bold;\n}\n\n.panel-group {\n  margin-left: -1rem;\n  padding-left: 1.2rem;\n  border-left: 6px solid #E8F1FE;\n}\n.panel-group .panel-heading {\n  background-color: #E8F1FE;\n  padding: 1rem;\n  margin: 0.5rem -1.5rem 0.5rem -1.5rem;\n  cursor: pointer;\n}\n.panel-group .panel-heading .panel-title {\n  font-size: 1.1rem;\n  font-weight: bold;\n  color: #1775F6;\n  margin: 0;\n}\n.panel-group .panel-heading .help-block {\n  margin-top: 0.5rem;\n}\n.panel-group .panel-heading i {\n  font-size: 1rem;\n}\n.panel-group .panel-heading[aria-expanded=true] .fa-chevron-up {\n  display: inline-block;\n}\n.panel-group .panel-heading[aria-expanded=true] .fa-chevron-down {\n  display: none;\n}\n.panel-group .panel-heading[aria-expanded=false] .fa-chevron-up {\n  display: none;\n}\n.panel-group .panel-heading[aria-expanded=false] .fa-chevron-down {\n  display: inline-block;\n}\n.panel-group .panel-collapse {\n  padding: 1rem 0 0.25rem 0;\n}\n\n#errorMessage {\n  margin-left: 15px;\n  box-sizing: border-box;\n  width: calc(100% - 30px);\n}\n#errorMessage p {\n  margin-bottom: 0;\n}\n\nselect[readonly].select2-hidden-accessible + .select2-container {\n  pointer-events: none;\n  touch-action: none;\n}\n\nselect[readonly].select2-hidden-accessible + .select2-container .select2-selection {\n  background: #eee;\n  box-shadow: none;\n}\n\nselect[readonly].select2-hidden-accessible + .select2-container .select2-selection__arrow, select[readonly].select2-hidden-accessible + .select2-container .select2-selection__clear {\n  display: none;\n}\n\n/*\n developer template form controls\n */\n.developer-template-controls-container {\n  overflow: auto;\n  max-height: calc(100vh - 400px);\n}\n.developer-template-controls-container .developer-template-controls {\n  display: flex;\n  gap: 16px;\n  margin-bottom: 6px;\n}\n.developer-template-controls-container .developer-template-controls .developer-template-placeholder {\n  width: 100%;\n  height: calc(100vh - 410px);\n  display: flex;\n  justify-content: center;\n  align-items: center;\n  font-size: 18px;\n  color: #0E4694;\n  background-color: #E8F1FE;\n  border-radius: 6px;\n}\n.developer-template-controls-container .developer-template-controls .developer-template-control-item {\n  background-color: #dcedff;\n  padding: 16px;\n  border-radius: 4px;\n  display: flex;\n  flex-direction: column;\n  align-items: center;\n  gap: 12px;\n  min-width: 360px;\n  max-width: 360px;\n}\n.developer-template-controls-container .developer-template-controls .developer-template-control-item .developer-template-control-form {\n  display: flex;\n  flex-direction: column;\n  flex: 1;\n  gap: 18px;\n}\n.developer-template-controls-container .developer-template-controls .developer-template-control-item .developer-template-control-form .xibo-form-input {\n  margin-bottom: 0.5rem;\n}\n.developer-template-controls-container .developer-template-controls .developer-template-control-item .developer-template-control-form .xibo-form-input label, .developer-template-controls-container .developer-template-controls .developer-template-control-item .developer-template-control-form .xibo-form-input .input-info-container {\n  float: left;\n}\n.developer-template-controls-container .developer-template-controls .developer-template-control-item .developer-template-control-form .player-compat-items {\n  display: grid;\n  grid-template-columns: repeat(4, 1fr);\n  gap: 3px 6px;\n  padding: 6px;\n  background-color: #B9D6FC;\n  border-radius: 6px;\n}\n.developer-template-controls-container .developer-template-controls .developer-template-control-item .developer-template-control-form .option-item {\n  display: inline-flex;\n  align-items: center;\n  gap: 6px;\n  background-color: #B9D6FC;\n  padding: 6px;\n  border-radius: 6px;\n  margin-bottom: 4px;\n}\n.developer-template-controls-container .developer-template-controls .developer-template-control-item .developer-template-control-form .test-container .test-title, .developer-template-controls-container .developer-template-controls .developer-template-control-item .developer-template-control-form .test-container .condition-title {\n  font-weight: bold;\n}\n.developer-template-controls-container .developer-template-controls .developer-template-control-item .developer-template-control-form .test-container .test-item {\n  background-color: #B9D6FC;\n  border-radius: 6px;\n  padding: 6px;\n  margin-bottom: 6px;\n}\n.developer-template-controls-container .developer-template-controls .developer-template-control-item .developer-template-control-form .test-container .test-item-properties {\n  display: flex;\n  gap: 6px;\n}\n.developer-template-controls-container .developer-template-controls .developer-template-control-item .developer-template-control-form .test-container .condition-item {\n  background-color: #d1e4fd;\n  padding: 6px;\n  border-radius: 8px;\n  margin-bottom: 6px;\n}\n.developer-template-controls-container .developer-template-controls .developer-template-control-item .developer-template-control-form .test-container .test-conditions-container {\n  margin: 6px 0 6px 34px;\n}\n.developer-template-controls-container .developer-template-controls .developer-template-control-item .developer-template-control-form .test-container .item-header {\n  display: flex;\n  justify-content: space-between;\n  margin-bottom: 6px;\n}\n.developer-template-controls-container .developer-template-controls .developer-template-control-item .developer-template-control-item-controls {\n  width: 100%;\n  display: flex;\n  justify-content: space-between;\n}\n.developer-template-controls-container .developer-template-controls .developer-template-control-item .developer-template-control-item-controls .item-move {\n  cursor: col-resize;\n  color: #74ACFA;\n}\n.developer-template-controls-container .developer-template-controls .developer-template-control-item .developer-template-control-item-controls .delete-btn {\n  padding: 0;\n  color: #BF5555;\n}\n.developer-template-controls-container .developer-template-controls .developer-template-control-item .developer-template-control-item-controls .item-move, .developer-template-controls-container .developer-template-controls .developer-template-control-item .developer-template-control-item-controls .delete-btn {\n  font-size: 24px;\n}\n\n.image-replace-control .image-replace-control-area {\n  height: 80px;\n  border-radius: 4px;\n  text-align: center;\n  color: #000000;\n  background-color: #F3F8FF;\n  padding: 18px;\n  border: 2px solid #c0daff;\n}\n.image-replace-control .image-replace-control-area.ui-droppable-active {\n  -webkit-touch-callout: none; /* iOS Safari */\n  -webkit-user-select: none; /* Safari */ /* Konqueror HTML */\n  -moz-user-select: none; /* Firefox */ /* Internet Explorer/Edge */\n  user-select: none; /* Non-prefixed version, currently supported by Chrome and Opera */\n  cursor: pointer;\n  position: relative;\n  z-index: 1011;\n}\n.image-replace-control .image-replace-control-area.ui-droppable-active.ui-droppable-hover, .image-replace-control .image-replace-control-area.ui-droppable-active:hover {\n  background-color: white;\n}\n\n.color-gradient {\n  background-color: #F3F8FF;\n  padding: 6px;\n  border-left: 8px solid #1775F6;\n}\n.color-gradient .color-gradient-colours {\n  display: flex;\n  gap: 6px;\n}\n.color-gradient .color-gradient-colours .picker-container.colorpicker.colorpicker-with-alpha {\n  min-width: 150px;\n}\n\n.fallback-data-content {\n  margin-bottom: 1rem;\n}\n.fallback-data-content .fallback-data-record {\n  padding: 1rem;\n  background-color: #fff;\n  border: 3px solid #74ACFA;\n  margin-bottom: 0.5rem;\n  border-radius: 0.5rem;\n  cursor: grab;\n}\n.fallback-data-content .fallback-data-record .fallback-data-record-previews {\n  margin-bottom: 0.6rem;\n}\n.fallback-data-content .fallback-data-record .fb-btn-edit, .fallback-data-content .fallback-data-record .fallback-data-record-previews {\n  display: block;\n}\n.fallback-data-content .fallback-data-record .fallback-data-record-preview {\n  display: flex;\n  justify-content: space-between;\n  margin-bottom: 0.2rem;\n}\n.fallback-data-content .fallback-data-record .fallback-data-record-preview .fallback-data-record-preview-title {\n  font-weight: bold;\n}\n.fallback-data-content .fallback-data-record .fallback-data-record-preview .fallback-data-record-preview-title, .fallback-data-content .fallback-data-record .fallback-data-record-preview .fallback-data-record-preview-value {\n  white-space: nowrap;\n  overflow: hidden;\n  text-overflow: ellipsis;\n}\n.fallback-data-content .fallback-data-record .fallback-data-record-preview.hidden {\n  display: none;\n}\n.fallback-data-content .fallback-data-record .fb-btn-save, .fallback-data-content .fallback-data-record .fallback-data-record-fields {\n  display: none;\n}\n.fallback-data-content .fallback-data-record.editing {\n  border-color: #EB7857;\n}\n.fallback-data-content .fallback-data-record.editing .fb-btn-edit, .fallback-data-content .fallback-data-record.editing .fallback-data-record-previews {\n  display: none;\n}\n.fallback-data-content .fallback-data-record.editing .fb-btn-save, .fallback-data-content .fallback-data-record.editing .fallback-data-record-fields {\n  display: block;\n}\n.fallback-data-content .fallback-data-record .fallback-property[data-is-required=true] {\n  color: #BF5555;\n  font-weight: bold;\n}\n.fallback-data-content .fallback-data-record .fallback-data-record-buttons {\n  display: flex;\n  justify-content: space-between;\n  gap: 1rem;\n}\n.fallback-data-content .fallback-data-record .fallback-data-record-buttons button {\n  flex: 1 1;\n}\n\n.button-switch-input-group button {\n  color: #1775F6;\n  border-color: #1775F6;\n}\n.button-switch-input-group button:not(.selected):hover {\n  color: #1775F6;\n  background-color: #c3dbfd;\n}\n.button-switch-input-group button.selected {\n  background-color: #1775F6;\n  color: #fff;\n}", "",{"version":3,"sources":["webpack://./ui/src/style/forms.scss","webpack://./ui/src/style/variables.scss","webpack://./ui/src/style/mixins.scss"],"names":[],"mappings":"AAAA;;;;;;;;;;;;;;;;;;;EAAA;AAyBA;EACI,YAAA;EACA,gBAAA;AAJJ;AAMI;EACI,YAAA;EACA,gBAAA;EACA,cCtBiB;ADkBzB;AAMQ;EACI,gBAAA;AAJZ;;AASA;EACI,aAAA;EACA,yBAAA;EACA,yBC9BqB;ED+BrB,kCAAA;AANJ;;AASA,eAAA;AACA;EACI,sBAAA;EACA,sBAAA;EACA,kBAAA;EACA,gBAAA;AANJ;AAQI;EACI,iBAAA;AANR;AASI;EACI,gBAAA;EACA,iBAAA;EACA,iBAAA;AAPR;AAUI;EACI,aAAA;EACA,kBAAA;EACA,UAAA;EACA,QAAA;AARR;AAUQ;EACI,cAAA;AARZ;AAWQ;EACI,aAAA;AATZ;AAaI;EACI,WAAA;EACA,YAAA;AAXR;AAaQ;EACI,wBAAA;AAXZ;AAeI;EACI,aAAA;AAbR;AAiBQ;EACI,cAAA;EACA,YAAA;AAfZ;AAiBY;EACI,YAAA;AAfhB;AAoBI;EACI,eAAA;EACA,WAAA;EACA,YAAA;EACA,UAAA;EACA,SAAA;EACA,aC9ByB;ED+BzB,sBC7Fe;AD2EvB;AAoBQ;EACI,gBAAA;AAlBZ;AAqBQ;EACI,cAAA;AAnBZ;AAsBQ;EACI,aAAA;AApBZ;AAuBQ;EACI,cAAA;AArBZ;;AA0BA;EACI,sBAAA;EACA,sBAAA;EACA,yBClHqB;EDmHrB,iBAAA;EACA,iBAAA;AAvBJ;;AA0BA;EACI,wBAAA;AAvBJ;;AA0BA,eAAA;AAIQ;EACI,+BAAA;EACA,kCAAA;AA1BZ;AA6BQ;EACI,uBAAA;EACA,eAAA;AA3BZ;;AAgCA,WAAA;AACA;EACI,iBAAA;EACA,iBAAA;EACA,cC/JiB;EDgKjB,iBAAA;AA7BJ;AA+BI;EACI,cC7JiB;ED8JjB,iBAAA;AA7BR;;AAiCA,oBAAA;AACA;EACI,yBCrJsB;ADuH1B;;AAgCA;EACI,wBAAA;AA7BJ;;AA+BA;EACI,wBAAA;AA5BJ;;AA+BA;EACI,wBAAA;EACA,uBAAA;EACA,qBAAA;EACA,oBAAA;EACA,sCAAA;EACA,qCAAA;EACA,cAAA;AA5BJ;AA8BI;EACI,aAAA;AA5BR;;AAgCA;EACI,qBAAA;EACA,sBAAA;EACA,aAAA;AA7BJ;;AAgCA;EACI,oBAAA;EACA,qBAAA;EACA,uBAAA;EACA,sBAAA;EACA,cAAA;AA7BJ;;AAgCA;EACI,WAAA;EACA,2BAAA;EACA,2BAAA;EACA,gBAAA;EACA,kCAAA;AA7BJ;AA+BI;EACI,aAAA;AA7BR;AAgCI;EACI,kBAAA;EACA,iBAAA;EACA,cAAA;AA9BR;AAgCQ;EACI,aAAA;AA9BZ;AAkCI;EACI,iBAAA;AAhCR;AAmCI;EACI,WAAA;EACA,gBAAA;EACA,OAAA;EACA,MAAA;EACA,UAAA;AAjCR;AAmCQ;EACI,2BAAA;AAjCZ;AAqCI;EACI,sBClPY;EDmPZ,oBAAA;EACA,2BAAA;AAnCR;AAuCQ;EACI,iCAAA;EACA,iBAAA;AArCZ;AAuCY;EACI,uBAAA;AArChB;AAyCQ;EACI,aAAA;AAvCZ;AA0CQ;EACI,2BAAA;EACA,2BAAA;EACA,uBAAA;EACA,iBAAA;AAxCZ;AA2CQ;EACI,aAAA;AAzCZ;AA4CQ;EACI,0BAAA;EACA,oBAAA;EACA,kBAAA;AA1CZ;AA6CQ;EACI,aAAA;AA3CZ;AA+CI;EACI,aCzNuC;ED0NvC,eAAA;EACA,UAAA;EACA,YAAA;EACA,kBAAA;EACA,qCAAA;EACA,sCAAA;EACA,gBAAA;AA7CR;AA+CQ;EACI,sCAAA;EACA,gBAAA;AA7CZ;AAgDQ;EACI,YAAA;AA9CZ;AAiDQ;EACI,mBCnSa;EDoSb,cC/Ra;ADgPzB;AAiDY;EACI,qBAAA;AA/ChB;AAmDQ;EACI,kCAAA;EACA,YAAA;EExSR,yBAAA;EACA,uCAAA;AFwPJ;AAkDY;EACI,WClTO;ADkQvB;;AAsDA;EACI,gBAAA;EACA,mBClUqB;EDmUrB,SAAA;EACA,aAAA;EACA,QAAA;EACA,mBAAA;EACA,yBAAA;AAnDJ;AAqDI;EACI,aAAA;AAnDR;AAsDI;EACI,SAAA;AApDR;AAsDQ;EACI,YAAA;EACA,WAAA;AApDZ;AAuDQ;EACI,aAAA;AArDZ;AAyDI;EACI,YAAA;EACA,YAAA;AAvDR;;AA4DA;EACI,aAAA;EACA,QAAA;AAzDJ;AA2DI;EACI,OAAA;EACA,YAAA;AAzDR;AA2DQ;EACI,yBAAA;EACA,iBAAA;AAzDZ;AA4DQ;EACI,sBAAA;EACA,oCAAA;AA1DZ;AA6DQ;EACI,wBAAA;AA3DZ;;AAiEA;EACI,eAAA;AA9DJ;AAgEI;EACI,WAAA;AA9DR;AAgEQ;EACI,yBAAA;EACA,iBAAA;EACA,cCxYW;AD0UvB;AAiEQ;EACI,aAAA;AA/DZ;AAkEQ;EACI,8BAAA;EACA,kBAAA;EACA,gBAAA;AAhEZ;AAkEY;EACI,qBAAA;AAhEhB;;AAwEI;EACI,cAAA;EACA,WAAA;AArER;AAuEI;EACI,0BAAA;EACA,iBAAA;EACA,uBAAA;EACA,yBC5ZiB;ADuVzB;AAuEQ;EACI,eAAA;AArEZ;AAwEQ;EACI,WAAA;EACA,aAAA;AAtEZ;AAyEQ;EACI,YAAA;AAvEZ;AAyEY;EACI,UAAA;EACA,eAAA;AAvEhB;AA0EY;EACI,UAAA;AAxEhB;AA4EQ;EACI,aAAA;EACA,QAAA;EACA,kBAAA;AA1EZ;AA4EY;EACI,UAAA;EACA,UAAA;AA1EhB;AA8EI;EACI,kBAAA;EACA,uBAAA;AA5ER;;AAkFI;EACI,0CAAA;AA/ER;AAmFQ;EACI,iBAAA;AAjFZ;AAoFQ;EACI,iBAAA;AAlFZ;AAqFQ;EACI,YAAA;AAnFZ;AAoFY;EACI,WAAA;AAlFhB;AAsFQ;EACI,YAAA;AApFZ;AAqFY;EACI,0BAAA;AAnFhB;AAuFQ;EACI,YAAA;AArFZ;AAsFY;EACI,0BAAA;AApFhB;AAyFI;EACI,gBAAA;EACA,OAAA;EACA,SAAA;EACA,aAAA;EACA,WAAA;EACA,6BAAA;AAvFR;;AA4FA;EACI,YAAA;EACA,yBCjgBiB;ADwarB;AA2FI;EACI,cC1fiB;ED2fjB,SAAA;AAzFR;AA2FQ;EACI,qBAAA;EACA,WAAA;EACA,UAAA;AAzFZ;AA4FQ;EACI,eAAA;EACA,YAAA;EACA,cAAA;EACA,kBAAA;AA1FZ;AA4FY;EACI,iBAAA;AA1FhB;AA6FY;EACI,yBAAA;AA3FhB;;AAiGA;EACI,kBAAA;EACA,YAAA;EACA,yBC1hBqB;ED2hBrB,cCvhBoB;EDwhBpB,mBAAA;EACA,gBAAA;EACA,uBAAA;AA9FJ;AAgGI;EACI,iBAAA;AA9FR;;AAmGA;EACI,iBAAA;AAhGJ;AAkGI;EACI,oCAAA;AAhGR;AAmGI;EACI,yBAAA;EACA,iBAAA;AAjGR;;AAsGA;EACI,kBAAA;EACA,oBAAA;EACA,8BAAA;AAnGJ;AAqGI;EACI,yBCvjBiB;EDwjBjB,aAAA;EACA,qCAAA;EACA,eAAA;AAnGR;AAqGQ;EACI,iBAAA;EACA,iBAAA;EACA,cCzkBS;ED0kBT,SAAA;AAnGZ;AAsGQ;EACI,kBAAA;AApGZ;AAuGQ;EACI,eAAA;AArGZ;AAyGY;EACI,qBAAA;AAvGhB;AAyGY;EACI,aAAA;AAvGhB;AA4GY;EACI,aAAA;AA1GhB;AA4GY;EACI,qBAAA;AA1GhB;AA+GI;EACI,yBAAA;AA7GR;;AAiHA;EACI,iBAAA;EACA,sBAAA;EACA,wBAAA;AA9GJ;AAgHI;EACI,gBAAA;AA9GR;;AAmHA;EACI,oBAAA;EACA,kBAAA;AAhHJ;;AAmHA;EACI,gBAAA;EACA,gBAAA;AAhHJ;;AAmHA;EACI,aAAA;AAhHJ;;AAmHA;;EAAA;AAGA;EACI,cAAA;EACA,+BAAA;AAhHJ;AAkHI;EACI,aAAA;EACA,SAAA;EACA,kBAAA;AAhHR;AAkHQ;EACI,WAAA;EACA,2BAAA;EACA,aAAA;EACA,uBAAA;EACA,mBAAA;EACA,eAAA;EACA,cCnpBa;EDopBb,yBChpBa;EDipBb,kBAAA;AAhHZ;AAmHQ;EACI,yBCtpBa;EDupBb,aAAA;EACA,kBAAA;EACA,aAAA;EACA,sBAAA;EACA,mBAAA;EACA,SAAA;EACA,gBAAA;EACA,gBAAA;AAjHZ;AAmHY;EACI,aAAA;EACA,sBAAA;EACA,OAAA;EACA,SAAA;AAjHhB;AAmHgB;EACI,qBAAA;AAjHpB;AAmHoB;EACI,WAAA;AAjHxB;AAqHgB;EACI,aAAA;EACA,qCAAA;EACA,YAAA;EACA,YAAA;EACA,yBCprBK;EDqrBL,kBAAA;AAnHpB;AAsHgB;EACI,oBAAA;EACA,mBAAA;EACA,QAAA;EACA,yBC5rBK;ED6rBL,YAAA;EACA,kBAAA;EACA,kBAAA;AApHpB;AAwHoB;EACI,iBAAA;AAtHxB;AAyHoB;EACI,yBCxsBC;EDysBD,kBAAA;EACA,YAAA;EACA,kBAAA;AAvHxB;AA0HoB;EACI,aAAA;EACA,QAAA;AAxHxB;AA2HoB;EACI,yBAAA;EACA,YAAA;EACA,kBAAA;EACA,kBAAA;AAzHxB;AA4HoB;EACI,sBAAA;AA1HxB;AA6HoB;EACI,aAAA;EACA,8BAAA;EACA,kBAAA;AA3HxB;AAgIY;EACI,WAAA;EACA,aAAA;EACA,8BAAA;AA9HhB;AAgIgB;EACI,kBAAA;EACA,cC9uBK;ADgnBzB;AAiIgB;EACI,UAAA;EACA,cCluBQ;ADmmB5B;AAkIgB;EACI,eAAA;AAhIpB;;AAwIA;EACI,YAAA;EACA,kBAAA;EACA,kBAAA;EACA,cCrvBsB;EDsvBtB,yBChwBoB;EDiwBpB,aAAA;EACA,yBAAA;AArIJ;AAuII;EE1wBA,2BAAA,EAAA,eAAA;EACA,yBAAA,EAAA,WAAA,EACA,mBAAA;EACA,sBAAA,EAAA,YAAA,EACA,2BAAA;EACA,iBAAA,EAAA,kEAAA;EFwwBI,eAAA;EACA,kBAAA;EACA,aCttBkB;ADqlB1B;AAmIQ;EAEI,uBAAA;AAlIZ;;AAwIA;EACI,yBCpxBoB;EDqxBpB,YAAA;EACA,8BAAA;AArIJ;AAuII;EACI,aAAA;EACA,QAAA;AArIR;AAuIQ;EACI,gBAAA;AArIZ;;AA2IA;EACI,mBAAA;AAxIJ;AA0II;EACI,aAAA;EACA,sBCryBe;EDsyBf,yBAAA;EACA,qBAAA;EACA,qBAAA;EACA,YAAA;AAxIR;AA0IQ;EACI,qBAAA;AAxIZ;AA2IQ;EACI,cAAA;AAzIZ;AA4IQ;EACI,aAAA;EACA,8BAAA;EACA,qBAAA;AA1IZ;AA4IY;EACI,iBAAA;AA1IhB;AA6IY;EACI,mBAAA;EACA,gBAAA;EACA,uBAAA;AA3IhB;AA8IY;EACI,aAAA;AA5IhB;AAgJQ;EACI,aAAA;AA9IZ;AAiJQ;EACI,qBCv1BQ;ADwsBpB;AAiJY;EACI,aAAA;AA/IhB;AAkJY;EACI,cAAA;AAhJhB;AAqJY;EACI,cC/0BY;EDg1BZ,iBAAA;AAnJhB;AAuJQ;EACI,aAAA;EACA,8BAAA;EACA,SAAA;AArJZ;AAuJY;EACI,SAAA;AArJhB;;AA6JI;EACI,cC33Ba;ED43Bb,qBC53Ba;ADkuBrB;AA4JQ;EACI,cC/3BS;EDg4BT,yBAAA;AA1JZ;AA6JQ;EACI,yBCp4BS;EDq4BT,WCv3BW;AD4tBvB","sourceRoot":""}]);
// Exports
...
/***/ "./node_modules/css-loader/dist/cjs.js!./node_modules/postcss-loader/dist/cjs.js??ruleSet[1].rules[3].use[2]!./node_modules/sass-loader/dist/cjs.js!./ui/src/style/help-pane.scss":
/*!****************************************************************************************************************************************************************************************!*\
  !*** ./node_modules/css-loader/dist/cjs.js!./node_modules/postcss-loader/dist/cjs.js??ruleSet[1].rules[3].use[2]!./node_modules/sass-loader/dist/cjs.js!./ui/src/style/help-pane.scss ***!
  \****************************************************************************************************************************************************************************************/
/***/ (function(module, __webpack_exports__, __webpack_require__) {
...
var ___CSS_LOADER_EXPORT___ = _node_modules_css_loader_dist_runtime_api_js__WEBPACK_IMPORTED_MODULE_1___default()((_node_modules_css_loader_dist_runtime_sourceMaps_js__WEBPACK_IMPORTED_MODULE_0___default()));
// Module
___CSS_LOADER_EXPORT___.push([module.id, "/*!\n * Copyright (C) 2023 Xibo Signage Ltd\n *\n * Xibo - Digital Signage - https://xibosignage.com\n *\n * This file is part of Xibo.\n *\n * Xibo is free software: you can redistribute it and/or modify\n * it under the terms of the GNU Affero General Public License as published by\n * the Free Software Foundation, either version 3 of the License, or\n * any later version.\n *\n * Xibo is distributed in the hope that it will be useful,\n * but WITHOUT ANY WARRANTY; without even the implied warranty of\n * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the\n * GNU Affero General Public License for more details.\n *\n * You should have received a copy of the GNU Affero General Public License\n * along with Xibo.  If not, see <http://www.gnu.org/licenses/>.\n */\n.help-pane {\n  position: fixed;\n  bottom: 0;\n  right: 10px;\n  z-index: 10000;\n}\n.help-pane-card {\n  width: 18rem;\n  display: none;\n}\n.help-pane-card .close-icon {\n  float: right;\n  padding-top: 3px;\n  cursor: pointer;\n}\n.help-pane-icon {\n  text-align: right;\n  padding: 15px;\n}\n.help-pane-icon i {\n  font-size: 48px;\n  color: #1775F6;\n  cursor: pointer;\n}", "",{"version":3,"sources":["webpack://./ui/src/style/help-pane.scss","webpack://./ui/src/style/variables.scss"],"names":[],"mappings":"AAAA;;;;;;;;;;;;;;;;;;;EAAA;AAsBA;EACE,eAAA;EACA,SAAA;EACA,WAAA;EACA,cAAA;AADF;AAGE;EACE,YAAA;EACA,aAAA;AADJ;AAGI;EACE,YAAA;EACA,gBAAA;EACA,eAAA;AADN;AAKE;EACE,iBAAA;EACA,aAAA;AAHJ;AAKI;EACE,eAAA;EACA,cC1Ce;ED2Cf,eAAA;AAHN","sourceRoot":""}]);
// Exports
/* harmony default export */ __webpack_exports__["default"] = (___CSS_LOADER_EXPORT___);
...
/* harmony import */ var _node_modules_css_loader_dist_cjs_js_node_modules_postcss_loader_dist_cjs_js_ruleSet_1_rules_3_use_2_node_modules_sass_loader_dist_cjs_js_forms_scss__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(/*! !!../../../node_modules/css-loader/dist/cjs.js!../../../node_modules/postcss-loader/dist/cjs.js??ruleSet[1].rules[3].use[2]!../../../node_modules/sass-loader/dist/cjs.js!./forms.scss */ "./node_modules/css-loader/dist/cjs.js!./node_modules/postcss-loader/dist/cjs.js??ruleSet[1].rules[3].use[2]!./node_modules/sass-loader/dist/cjs.js!./ui/src/style/forms.scss");
...
var update = _node_modules_style_loader_dist_runtime_injectStylesIntoStyleTag_js__WEBPACK_IMPORTED_MODULE_0___default()(_node_modules_css_loader_dist_cjs_js_node_modules_postcss_loader_dist_cjs_js_ruleSet_1_rules_3_use_2_node_modules_sass_loader_dist_cjs_js_forms_scss__WEBPACK_IMPORTED_MODULE_6__["default"], options);




       /* harmony default export */ __webpack_exports__["default"] = (_node_modules_css_loader_dist_cjs_js_node_modules_postcss_loader_dist_cjs_js_ruleSet_1_rules_3_use_2_node_modules_sass_loader_dist_cjs_js_forms_scss__WEBPACK_IMPORTED_MODULE_6__["default"] && _node_modules_css_loader_dist_cjs_js_node_modules_postcss_loader_dist_cjs_js_ruleSet_1_rules_3_use_2_node_modules_sass_loader_dist_cjs_js_forms_scss__WEBPACK_IMPORTED_MODULE_6__["default"].locals ? _node_modules_css_loader_dist_cjs_js_node_modules_postcss_loader_dist_cjs_js_ruleSet_1_rules_3_use_2_node_modules_sass_loader_dist_cjs_js_forms_scss__WEBPACK_IMPORTED_MODULE_6__["default"].locals : undefined);
...
/* harmony import */ var _node_modules_style_loader_dist_runtime_styleTagTransform_js__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(/*! !../../../node_modules/style-loader/dist/runtime/styleTagTransform.js */ "./node_modules/style-loader/dist/runtime/styleTagTransform.js");
/* harmony import */ var _node_modules_style_loader_dist_runtime_styleTagTransform_js__WEBPACK_IMPORTED_MODULE_5___default = /*#__PURE__*/__webpack_require__.n(_node_modules_style_loader_dist_runtime_styleTagTransform_js__WEBPACK_IMPORTED_MODULE_5__);
/* harmony import */ var _node_modules_css_loader_dist_cjs_js_node_modules_postcss_loader_dist_cjs_js_ruleSet_1_rules_3_use_2_node_modules_sass_loader_dist_cjs_js_help_pane_scss__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(/*! !!../../../node_modules/css-loader/dist/cjs.js!../../../node_modules/postcss-loader/dist/cjs.js??ruleSet[1].rules[3].use[2]!../../../node_modules/sass-loader/dist/cjs.js!./help-pane.scss */ "./node_modules/css-loader/dist/cjs.js!./node_modules/postcss-loader/dist/cjs.js??ruleSet[1].rules[3].use[2]!./node_modules/sass-loader/dist/cjs.js!./ui/src/style/help-pane.scss");
...
var update = _node_modules_style_loader_dist_runtime_injectStylesIntoStyleTag_js__WEBPACK_IMPORTED_MODULE_0___default()(_node_modules_css_loader_dist_cjs_js_node_modules_postcss_loader_dist_cjs_js_ruleSet_1_rules_3_use_2_node_modules_sass_loader_dist_cjs_js_help_pane_scss__WEBPACK_IMPORTED_MODULE_6__["default"], options);




       /* harmony default export */ __webpack_exports__["default"] = (_node_modules_css_loader_dist_cjs_js_node_modules_postcss_loader_dist_cjs_js_ruleSet_1_rules_3_use_2_node_modules_sass_loader_dist_cjs_js_help_pane_scss__WEBPACK_IMPORTED_MODULE_6__["default"] && _node_modules_css_loader_dist_cjs_js_node_modules_postcss_loader_dist_cjs_js_ruleSet_1_rules_3_use_2_node_modules_sass_loader_dist_cjs_js_help_pane_scss__WEBPACK_IMPORTED_MODULE_6__["default"].locals ? _node_modules_css_loader_dist_cjs_js_node_modules_postcss_loader_dist_cjs_js_ruleSet_1_rules_3_use_2_node_modules_sass_loader_dist_cjs_js_help_pane_scss__WEBPACK_IMPORTED_MODULE_6__["default"].locals : undefined);
...
// --- Xibo JS files ----
// Xibo forms
__webpack_require__(/*! ./src/style/forms.scss */ "./ui/src/style/forms.scss");
__webpack_require__(/*! ./src/core/forms.js */ "./ui/src/core/forms.js");

// Xibo help
__webpack_require__(/*! ./src/core/help-pane.js */ "./ui/src/core/help-pane.js");
__webpack_require__(/*! ./src/style/help-pane.scss */ "./ui/src/style/help-pane.scss");

// Xibo datatables and folders
__webpack_require__(/*! ./src/core/xibo-datatables.js */ "./ui/src/core/xibo-datatables.js");

// Xibo calendar
__webpack_require__(/*! ./src/core/xibo-calendar.js */ "./ui/src/core/xibo-calendar.js");

// Xibo core
__webpack_require__(/*! ./src/core/xibo-cms.js */ "./ui/src/core/xibo-cms.js");
/******/ })()
;
//# sourceMappingURL=xibo.bundle.min.js.map