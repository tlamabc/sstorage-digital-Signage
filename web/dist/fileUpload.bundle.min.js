...
window.openUploadForm = function (options) {
  ...
  setTimeout(function () {
    ...
    form.fileupload(uploadOptions).bind('fileuploadsubmit', function (e, data) {
...
function checkImagePixelSize(e, data) {
  const files = data === undefined ? e.currentTarget.files : data.files;
  const $existingFiles = $('.template-upload canvas').closest('tr').find('td span.info');
  const checkExist = setInterval(function () {
        ...
        if (!file.error && file.type.includes('image')) {
          ...
          img.onload = function () {
            if (this.width > maxImagePixelSize || this.height > maxImagePixelSize) {
              ...
              $('.template-upload canvas').closest('tr').find('td span.info')[index].append(helpText);
            }
...
/******/ })()
;
//# sourceMappingURL=fileUpload.bundle.min.js.map