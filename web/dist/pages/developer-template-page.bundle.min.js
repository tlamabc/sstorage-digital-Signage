...
window.moduleTemplateAddFormOpen = function (dialog) {
  ...
  $('#dataType', $form).on('select2:select', function (e) {
    const dataType = $(e.currentTarget).select2('data')[0].id;
    const $templateSelect = $(dialog).find('#copyTemplateId');
    const searchUrl = moduleTemplateSearchURL.replace(':dataType', dataType);
...
function makeTemplateSelect($element) {
  ...
  $.ajax({
    ...
    error: function (xhr) {
      SystemMessage(xhr.message || developerTemplatePageTrans.unknownError, false);
    }
  });
}
...
/******/ })()
;
//# sourceMappingURL=developer-template-page.bundle.min.js.map